...
subroutine ew_startup(natom_local,iblo,inb,x,ix)
   ...
   use pol_gauss_mdin, only : ipgm
   use pol_gauss_multipoles, only : betamax
   use external_module, only: pme_external_only_overrides_bonded, pme_external_molecule_idx
   implicit none
...
!ENDYDEBUG
   ...
   if (pme_external_only_overrides_bonded) then
       call load_adj_mask_bonded(iblo, inb, natom, mxadjmsk, ix(imask1), ix(imask2), numadjst, pme_external_molecule_idx)
   endif
...
subroutine load_adj_mask_bonded( &
    num_excluded_atoms, &
    excluded_atoms_idxs, &
    num_atoms, &
    max_adj_mask, &
   ...
   integer, intent(in) :: num_excluded_atoms(*), excluded_atoms_idxs(*)
   integer, intent(in) :: num_atoms, max_adj_mask
   integer, intent(in) :: molecule_idx(*)
   integer, intent(inout) :: adj_mask1(*), adj_mask2(*), num_total_adj

   integer :: atom_i, atom_j, offset
   integer :: excluded_counter, num_adj
   logical :: is_excluded

   ! Each extra atom to exclude increases the num_adj counter
   ! Before filling the adj_mask, check if num_adj will overflow the max_adj_mask
   offset = 0
   num_adj = 0
   do atom_i = 1, num_atoms - 1
      do atom_j = atom_i + 1, num_atoms
         if (molecule_idx(atom_i) == molecule_idx(atom_j)) then
            ! Both atoms belong to the same molecule
            ! Check whether they have already been excluded
            is_excluded = .false.
            do excluded_counter = 1, num_excluded_atoms(atom_i)
               if (excluded_atoms_idxs(offset + excluded_counter) == atom_j) then
                  is_excluded = .true.
               end if
            end do
            if (.not. is_excluded) then
               ! Exclude atoms
               num_adj = num_adj + 1
               if (num_total_adj + num_adj > max_adj_mask) then
                  call sander_bomb('exclude_same_molecule_pairs', 'MAX_ADJ_MASK is too small!', ' ')
               end if
               adj_mask1(num_total_adj + num_adj) = atom_i
               adj_mask2(num_total_adj + num_adj) = atom_j
            end if
         end if
      end do
      offset = offset + num_excluded_atoms(atom_i)
   end do
   num_total_adj = num_total_adj + num_adj
end subroutine load_adj_mask_bonded
...
subroutine read_ewald(ax,bx,cx,alphax,betax,gammax)
   use constants, only : RETIRED_INPUT_OPTION, one, two, half
...
#  include "parallel.h"
   ...
   !       also different depending on ntr (positional restraints):
   if (netfrc .eq. NO_INPUT_VALUE) then
      if ((imin .eq. 0 .or. imin .eq. 6) .and. ntr .eq. 0 ) then
         netfrc = 1         
      else        
         netfrc = 0
      end if      
   else
      if ((imin .ne. 0 .or. ntr .ne. 0) .and. netfrc .ne. 0) then
#ifndef API
         write(6,'(a)') '| Setting netfrc to 0 for minimization or constraints'
#endif
         netfrc = 0                                           
      end if
   end if
   ...
   ! RL: For pGM ewald, a/b/c are enclarged to make sure their
   ! repsective grid spacings are less or equal to 1 Angstrom.
   ! In compute_nfft, the trial starts at int(x) - 1, so we need to
   ! add one to b/c to make sure it is greater than or equal to b/c
   if ( nfft1 == 0 ) then
      !        RCFFT needs an even dimension for x direction
      if ( ipgm == 0 ) then
         call compute_nfft((a + one)*half   ,nfft1)
      else
         call compute_nfft((a + two)*half   ,nfft1)
      end if
      nfft1=nfft1*2
   end if
   if ( ipgm == 0 ) then
      ...
      if ( nfft2 == 0 ) call compute_nfft(b + one,nfft2)
      if ( nfft3 == 0 ) call compute_nfft(c + one,nfft3)
   end if
