...
module nfe_cv_ORIENTATION_PROJ
...
#  include "nfe-mpi.h"
   ...
   nfe_assert(error.eq.0)
      ...
      tmp = 2.0 * 2.0 * (priv%quaternion(1,1)) * fcv                           !XX! FIX ME! needs to fix some conditions
...
#  include "nfe-mpi.h"
   ...
   if (cv_nr.eq.0) then
      if (associated(coor)) &
          ...
          if (error.ne.0) &
           NFE_OUT_OF_MEMORY
           call read_refcrd(coor, trim(refcrd_file))
...
end module nfe_cv_ORIENTATION_PROJ
