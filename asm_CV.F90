...
contains
    ...
    subroutine CV_bond( x, atm, bond, gradient )
        ...
        _REAL_, intent(out) :: bond
        _REAL_, dimension(:), intent(out) :: gradient
    ...
    subroutine CV_angle( x, atm, angle, gradient )
        ...
        _REAL_, dimension(3,3) :: xatm, dx
        _REAL_, dimension(3) :: ab, bc, nab, nbc
        _REAL_ :: dab, dbc, cosang, d_acos
        integer :: i
    ...
    subroutine CV_dihedral( x, atm, dihedral, gradient ) !Taken from fdynamo 2.2 with minor changes
        ...
        _REAL_, intent(out) :: dihedral
        _REAL_, dimension(:), intent(out) :: gradient
        _REAL_, dimension(3,4) :: xatm
        integer :: i

        _REAL_ :: df, diff, cosang, factij, factkl, msize, nsize, rkj, sgnfac
        _REAL_, dimension(3) :: drij, drkj, drkl, dti, dtj, dtk, dtl, m, n
...
!         flush(6)
    ...
    subroutine CV_pointplane( x, atm, pointplane, gradient )
        ...
        _REAL_, dimension(:), intent(in) :: x
        integer, dimension(4), intent(in) :: atm
        _REAL_, intent(out) :: pointplane
        _REAL_, dimension(:), intent(out) :: gradient
        _REAL_, dimension(3,4) :: xatm

        _REAL_,dimension(3)    :: V, U, W                   !Vectores y puntos definicion
        _REAL_,dimension(3)    :: P                         !Puntos definicion
        integer                :: i

        !PLANO
        _REAL_,dimension(4)     :: Pl                        !Ecuacion del plano
        _REAL_                  :: E, F                      !Escalares auxiliares
        _REAL_,dimension(3,4,4) :: gPl                       !Gradiente de la Ec del plano
        _REAL_,dimension(3,4)   :: gE, gF, g                 !Gradientes
...
!========Local subroutines========
	...
	_REAL_, dimension(3), intent(in) :: A, B
	_REAL_, intent(out) :: value
	_REAL_, dimension(3), intent(out) :: A_gradient, B_gradient

	_REAL_, dimension(3) :: dx
...
#include "box.h"
        _REAL_, dimension(3), intent(inout) :: dx
        integer :: i
