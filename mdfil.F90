...
#  include "../include/dprec.fh"
   ...
   logical :: cpin_specified     = .false.
   logical :: cpout_specified    = .false.
   logical :: cprestrt_specified = .false.
   logical :: cein_specified     = .false.
   logical :: ceout_specified    = .false.
   logical :: cerestrt_specified = .false.
   logical :: cpeout_specified   = .false.
...
!scaledMD log file
   ...
   do while (iarg < last_arg_index)
      ...
      else if (arg == '-cein') then
         cein_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,cein)
      else if (arg == '-ceout') then
         ceout_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,ceout)
      else if (arg == '-cerestrt') then
         cerestrt_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,cerestrt)
      else if (arg == '-ce-data') then
         write_dump_e = .true.
         ce_dump_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg, ce_dump)
      else if (arg == '-cpein') then
         cpein_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,cpein)
      else if (arg == '-cpeout') then
         cpeout_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,cpeout)
      else if (arg == '-cperestrt') then
         cperestrt_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg,cperestrt)
      else if (arg == '-cphe-data') then
         write_dump_ph = .true.
         cphe_dump_specified = .true.
         iarg = iarg + 1
         call getarg_wrap(iarg, cphe_dump)
...
!scaledMD log file
         ...
         iarg = iarg + 1
         call getarg_wrap(iarg, ipi_hostname)
      else if (arg == '-port') then
         iarg = iarg + 1
         call getarg_wrap(iarg,arg)
         read(arg,'(i7)',err=93) ipi_port
  ...
  if (cpein_specified) then
    if (cpin_specified) then
      write(6, '(a)') 'ERROR: The cpein file cannot be specified together with the cpin file!'
      call mexit(6, 1)
    end if
    if (cein_specified) then
      write(6, '(a)') 'ERROR: The cpein file cannot be specified together with the cein file!'
      call mexit(6, 1)
    end if
  end if
   ...
   if (suffix_specified) then
      ...
      if (.not. ceout_specified .or. always_add_suffix) &
         call add_suffix(ceout, outfile_suffix)

      if (.not. cerestrt_specified .or. always_add_suffix) &
         call add_suffix(cerestrt, outfile_suffix)

      if (.not. cpeout_specified .or. always_add_suffix) &
         call add_suffix(cpeout, outfile_suffix)

      if (.not. cperestrt_specified .or. always_add_suffix) &
         call add_suffix(cperestrt, outfile_suffix)
      ...
      if (.not. ce_dump_specified .or. always_add_suffix) &
         call add_suffix(ce_dump, outfile_suffix)
   ...
   93 write(6,*) 'mdfil: Error "-port" requires integer argument'
   call mexit(6, 1)
   ...
   9000 format(/,5x, &
         ...
         '-host ipi_hostname -port ipi_port' &
...
!        '-cein CEDAT       Constant Redox potential state information ',
!        '-cerstrt CEDAT    Constant Redox potential state restart information',
!        '-ceout CEOUT      Constant Redox potential protonation output
!        '-cpein CPEDAT     Constant pH and Redox potential state information ',
!        '-cperstrt CPEDAT  Constant pH and Redox potential state restart information',
!        '-cpeout CPEOUT    Constant pH and Redox potential protonation output
!        '-evbin  EVBIN     EVB input file ',
!        '-evbout EVBOUT    EVB output file ',
