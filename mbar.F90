...
!===========================================
    ...
    subroutine setup_mbar_parallel()
      !
      ! setup_mbar is only called on sanderrank 0
      ! setup_mbar_parallel allocates memory for sanderranks > 0
      ! This should be called in parallel.F90, after the other bar
      ! variables are bcast.
      !
      call mpi_bcast(bar_states,1,MPI_INTEGER,0,commsander,ierr)
      ...
      if ( ifmbar .ne. 0 ) then
         ...
         if (sanderrank>0) then
            ...
            allocate (bar_lambda(bar_states),stat=ier_alloc)
            if (ier_alloc /= 0) call sander_bomb('allocate(2) bar_lambda[mbar.f]','cant allocate','')
            allocate (bar_cont(bar_states),stat=ier_alloc)
            if (ier_alloc /= 0) call sander_bomb('allocate(2) bar_cont[mbar.f]','cant allocate','')
            allocate (bar_cont_partner(bar_states),stat=ier_alloc)
            if (ier_alloc /= 0) call sander_bomb('allocate(2) bar_cont_partner[mbar.f]','cant allocate','')

            bar_cont(1:bar_states) = 0.d0
            bar_cont_partner(1:bar_states) = 0.d0
            
         end if

         call mpi_bcast(bar_lambda, bar_states, MPI_DOUBLE_PRECISION, &
              & 0, commsander, ierr)
         
      end if
      
    end subroutine setup_mbar_parallel
    ...
    subroutine calc_mbar_energies(etot, etot_partner)
      ...
      if (sanderrank==0) then
         ...
         do i=1, bar_states
            ...
            energies(i) = energy
            write (6,'(a,f6.4,a,f16.6)') 'Energy at ', bar_lambda(i), ' = ', energy
         end do
         write(6,'(t2,78("-"),/)')
         bar_cont(1:bar_states) = 0.0d0
      end if
