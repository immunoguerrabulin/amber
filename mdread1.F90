...
#  define FATAL_ERROR call mexit(6, 1)
   ...
   use md_scheme, only: ischeme, ithermostat, therm_par, therm_vol, baro_stochastic
...
#  endif /* API */
   ...
   use qmmm_fires_module, only : fires, fire_inner, fire_outer, fire_inner_solvent, fires_k, mts_fires, mts_n  !JOSE MOD

   
   use dprcmod, only : dprc_read_mdin
...
#  include "ew_erfc_spline.h"
   ...
   integer     itotst
   integer     inerr
   logical     mdin_cntrl, mdin_lmod, mdin_qmmm, mdin_dprc, mdin_fireb  ! true if namelists are in mdin
   logical     mdin_sebomd
...
!         ../pysander/sander/src/{pysandermodule,pysandermoduletypes}.c
   ...
   namelist /cntrl/ irest,ibelly, &
         ...
         igamd, &
         ischeme, ithermostat, therm_par, therm_vol, baro_stochastic, &
         scaledMD,scaledMD_lambda, &
         iemap,gammamap, &
         isgld,isgsta,isgend,sgtype,sgsize,tsgavg,sgft,sgff,sgfg,tempsg,tsgavp,sgmask, &
         jar, iamoeba, ipgm, &
         ...
         ala,arg,asn,asp,cys,gln,glu,gly,his,hip,ile,leu,lys,met,phe,pro,ser,thr,triptophan,tyr,valine,bb, &
         ...
         !
         fire_inner,fire_inner_solvent,fire_outer, &
         !
...
#ifdef API
   !JOSE MOD: API mapping for FIRES MTS control
   mts_fires = 1
#endif
#ifdef API
   ! JOSE MOD: map API-provided FIRES inputs into module variables
   fires      = input_options%fires
   fires_k    = input_options%fires_k
   fire_inner = trim(input_options%fire_inner)
   fire_inner_solvent = trim(input_options%fire_inner_solvent)
   fire_outer = trim(input_options%fire_outer)
   if (input_options%mts_fires > 0) mts_fires = input_options%mts_fires
#endif
...
#endif
#ifdef API
   if (input_options%mts_fires > 0.0d0) mts_fires = input_options%mts_fires  ! ps
   if (input_options%mts_n > 0)        mts_n     = input_options%mts_n
#endif
   mdin_ewald=.false.
   mdin_pb=.false.
#ifdef APBS
...
!  ntp = 2 anisotropic directional pressure scaling
   ...
   ala = input_options%ala
   arg = input_options%arg
   asn = input_options%asn
   asp = input_options%asp
   cys = input_options%cys
   gln = input_options%gln
   glu = input_options%glu
   gly = input_options%gly
   his = input_options%his
   hip = input_options%hip
   ile = input_options%ile
   leu = input_options%leu
   lys = input_options%lys
   met = input_options%met
   phe = input_options%phe
   pro = input_options%pro
   ser = input_options%ser
   thr = input_options%thr
   triptophan = input_options%triptophan
   tyr = input_options%tyr
   valine = input_options%valine
   bb = input_options%bb
...
#  endif
   ...
   call nmlsrc('fireb', 5, ifind)
   if (ifind /= 0) mdin_fireb = .true.
   rewind 5
...
#  endif /* APBS */
   ...
   call nmlsrc('dprc',5,ifind)
   if (ifind /= 0) mdin_dprc=.true.
   rewind 5
   ...
   if (ischeme == 1) then
      ...
      if (therm_par < 0d0) then
         write(6,'(1x,a,/)') 'Middle scheme: the parameter therm_par MUST be specified and be non-negative'
         FATAL_ERROR
      end if
      if (ntt > 0) then
      ...
      if (ntp > 3 .and. ipimd == 0) then
         write(6,'(1x,a,/)') 'Middle scheme: can not be used with ntp > 3'
         FATAL_ERROR
      endif
      if (ntp > 1 .and. ipimd > 0) then
         write(6,'(1x,a,/)') 'Middle scheme: can not be used with ntp > 2 and PIMD'
         FATAL_ERROR
         if (therm_vol < 0d0) then
           write(6,'(1x,a,/)') 'Middle scheme: the parameter therm_vol MUST be specified and be non-negative'
           FATAL_ERROR
         end if
      endif
      ...
      if (therm_vol > 0) then
         write(6,'(1x,a,/)') 'therm_vol should be used with ischeme > 0)'
         FATAL_ERROR
      endif
   else
      write(6,'(1x,a,/)') &
...
#ifdef MPI
   ! Jarzynski analysis valid options. Set reasonable defaults for related
   ! control parameters if unset.
   if ( ijarzynski > 0 ) then
      inerr = 0
      if ( icfe /= 1 ) then
         write(6, '(/2x,a)') 'icfe must be set to 1 if ijarzynski = 1'
         inerr = 1
      end if

      if ( ntave <= 0 ) then
         write(6, '(/2x,a)') 'ntave must be > 0 if ijarzynski = 1'
         inerr = 1
      end if

      if ( ntave > nstlim ) then
         write(6, '(/2x,a)') 'ntave must be <= nstlim if ijarzynski = 1'
         inerr = 1
      end if

      if ( ntave > 0 ) then
         if ( mod(nstlim,ntave) /= 0 ) then
            write(6, '(/2x,2a)') 'nstlim must be a multiple of ntave if ', &
                 & 'ijarzynski = 1'
            inerr = 1
         end if
      end if
      ...
      if ( dynlmb < 0.d0 ) then
         write(6, '(/2x,a)') 'dynlmb must be >= 0.0 if ijarzynski = 1'
         inerr = 1
      end if
      ...
      ! if ( clambda < clambda_min ) then
      !    write(6, '(/2x,2a)') 'clambda must be >= 0.0 (if ifsc=0) or >= 0.005 (if ifsc=1)', &
      !         &  ' when softcore potentials are used'
      !    inerr = 1
      ! end if
      
      if ( clambda >= clambda_max ) then
         write(6, '(/2x,2a)') 'clambda must be < 1.0 (if ifsc=0) or < 0.995 (if ifsc=1)', &
              &  ' when ijarzynski = 1'
         inerr = 1
      end if

      ! Nchanges = nstlim/ntave
      ! dynlmb = dL/change
      ! total change = dynlmb * Nchanges = dynlmb * nstlim/ntave
      if ( clambda + (dynlmb*nstlim/ntave) > clambda_max ) then
         write(6, '(/2x,2a)') 'clambda + (dynlmb*ntave/nstlim) should not ', &
              & 'be larger than 1.0 (if ifsc=0) or 0.995 (if ifsc=1) when ijarzynski = 1'
         inerr = 1
      end if
      ...
      if ( inerr == 0 ) then
         if ( dynlmb < 1.d-10 ) then
            ! (lambda_max - lambda_min) / Nchanges = (lambda_max - lambda_min) * ntave/nstlim
            dynlmb = (clambda_max - clambda) * ntave/nstlim
            write(6, '(/2x,a,es16.6)') 'NOTE: ijarzynski=1 is resetting dynlmb = ',&
                 & dynlmb
         end if
         jarzynski_lambda_min = clambda
         ! total change = dynlmb * Nchanges = dynlmb * nstlim/ntave
         jarzynski_lambda_max = clambda + (dynlmb*nstlim/ntave)
      end if
...
! ASM
   ...
   if (asm /= 0 .and. asm /= 1) then
      write(6,*) ' asm must be either 0 or 1'
      FATAL_ERROR
   end if
#endif MPI
