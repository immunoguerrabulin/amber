...
subroutine ewald_force(crd,numatoms,iac,ico,charge, &
      cn1,cn2,cn6,cn7,cn8,C4Pairwise,eelt,epol,frc,x,ix,ipairs, & !New2021
...
#define MPI_DOUBLE_PRECISION MPI_REAL8
   ...
   integer numatoms,iac(*),ico(*),cn7(*),C4Pairwise
   _REAL_ crd(3,*),charge(*),cn1(*),cn2(*),cn6(*), cn8(*), &
          eelt,epol,frc(3,*),xr(3,*),virvsene ! New2021
#ifdef HAS_10_12
   _REAL_ asol(*), bsol(*)
#endif
   _REAL_ pol(*), pol2(*)
...
!
   ...
   if( use_pme /= 0 .and. mod(irespa,nrespa) == 0) then
      if(eedmeth /= 4 .and. eedmeth /= 7) then 
         !--------------------------------------------------------
         ! SELF ENERGY
         !--------------------------------------------------------
         ...
         if (.not. qmmm_nml%ifqnt) then
           ...
           call adj_mm_link_pair_crd(crd) !Replace the MM atom's coordinates with the link atom.


           ! chargesum=0.d0
           ! do i=1,numatoms
           !    chargesum = chargesum + charge(i)
           ! end do
           ! write(6,'(A,es20.10)')"chargesum",chargesum
           ! do i=1,16
           !    write(6,'(a,i6,4f15.10)')"charge",i,crd(1:3,i),charge(i)
           ! end do
         ...
         call timer_stop(TIME_REC)
      end if !effectively switched off PME
      !-------------------------------------------------------
   ...
   if(i_do_direct)then
      ...
      call get_nb_energy(iac,ico,ntypes,charge, &
            cn1,cn2,cn6,cn7,cn8,C4Pairwise,frc,numatoms, ipairs, & !New2021
...
!
   ...
   !--------------------------------------------------------
   !! SNS: not sure how these should be dealt with when eedmeth==4; 
   !! Right now I don't have the corresponding example
   if ( mpoltype == 0 )then
