...
#  define DELAYED_ERROR inerr = 1
   ...
   use findmask
#ifdef LES
   use genbornles, only: isnucat, surf_corr ! gbneck2nu: check if atom belongs to nuc or protein
#else
   use genborn, only: isnucat, surf_corr ! gbneck2nu: check if atom belongs to nuc or protein
#endif
   ...
   use md_scheme, only: ischeme, therm_par, baro_stochastic
...
use sgld, only : isgld,sgtype ! for RXSGLD
   ...
   use softcore, only : ifsc, scalpha, scbeta, dvdl_norest, &
                        sceeorder, logdvdl, dynlmb, ijarzynski
...
! REMD
   ...
   use trajenemod, only: orig_ig
   
   implicit none
   _REAL_ x(*)
   _REAL_ surf_ten, surf_ten_bb
#  include "../include/memory.h"
   ...
   integer iwrap_maskgp( natom ) , ier, tfe_flag
   logical errFlag
   _REAL_ emtmd
#ifndef LES
   logical newstyle
...
#  include "multitmd.h"
   ...
   if (ischeme>0 .and. ipimd.eq.2) then
      nscm = 0
      ndfmin = 0
   endif
...
#  ifdef MPI /* SOFT CORE */
   if( icfe /= 0 .or. ifsc/=0) then
      ...
      if ( ijarzynski > 0 ) then
         write(6,'(5x,a,i8)')       'ijarzynski =', ijarzynski
      end if
   end if
   if ( ifmbar /= 0 ) then
...
#  endif /* MPI */
   ...
   ! External library information. Possible options are (1, 2).
   ! 1 adds the external potential to the FF
   ! 2 overrides the FF
   if (iextpot .ne. 0) then
     write(6, '(/a)') 'External library options:'
     if (.not. (iextpot == 1 .or. iextpot == 2)) then
        DELAYED_ERROR
     end if
     if (iextpot == 2) then
        write(6, '(/a)') 'Overriding force field with external potential'
     endif
...
!---- QMMM Options ----
   ...
   if( qmmm_nml%ifqnt ) then
      ...
      if ( qmmm_nml%qmtheory%PM3 ) then
         ...
         write(6, '(5x,"     qm_theory = XTB")',ADVANCE='NO')
      else if ( qmmm_nml%qmtheory%ISDFTBPLUS ) then
         write(6, '(5x,"     qm_theory = DFTBPLUS")',ADVANCE='NO')
      else
      ...
      EXTERN: if ( .not. qmmm_nml%qmtheory%EXTERN &
           & .and. .not. qmmm_nml%qmtheory%ISQUICK &
           & .and. .not. qmmm_nml%qmtheory%ISTCPB &
           & .and. .not. qmmm_nml%qmtheory%ISXTB &
           & .and. .not. qmmm_nml%qmtheory%ISDFTBPLUS ) then
...
#  endif /* MPI */
   ...
   if( gbsa == 3 ) then
      ...
      !FATAL_ERROR
      write(6,*) 'using GBSA=3, the pwSASA model'
   end if
#ifdef APBS
     ...
     !       put fs(i)*(rborn(i) - offset) into the "fs" array
      ...
      if ( gbsa == 1 ) then
         ...
         do i=1,natom
            ...
            nbond=ix(i80+i-1)
           if (tfe_flag == 1) then  
           call surf_corr(surf_ten,surf_ten_bb,surften,i,ala,arg,asn,asp,cys,gln,glu,gly,his,hip,ile,leu,lys,met,phe,pro,ser,thr,triptophan,tyr,valine,bb,nres,23, &
                  ix(i02:i02+nres-1),ih(m02:m02+nres-1)) 
            else
            surf_ten = 1
            surf_ten_bb = 1
            end if           
            if (atomicnumber .eq. 6) then
               if (hybridization .eq. 4) then
                  if (nbond == 1) then
                     ...
                     x(l170-1+i) = 0.77887d0*surf_ten
                     x(l175-1+i) = -0.28063d0*surf_ten
                     x(l180-1+i) = -0.0012968d0*surf_ten
                     x(l185-1+i) = 0.00039328d0*surf_ten
                  else if (nbond == 2) then
                     x(l165-1+i) = 1.70d0 + 1.4d0
                     x(l170-1+i) = 0.56482d0*surf_ten_bb
                     x(l175-1+i) = -0.19608d0*surf_ten_bb
                     x(l180-1+i) = -0.0010219d0*surf_ten_bb
                     x(l185-1+i) = 0.0002658d0*surf_ten_bb
                  else if (nbond == 3) then
                     x(l165-1+i) = 1.70d0 + 1.4d0
                     x(l170-1+i) = 0.23348d0*surf_ten
                     x(l175-1+i) = -0.072627d0*surf_ten
                     x(l180-1+i) = -0.00020079d0*surf_ten
                     x(l185-1+i) = 0.00007967d0*surf_ten
                  ...
                  if (nbond == 2) then
                     x(l165-1+i) = 1.70d0 + 1.4d0
                     x(l170-1+i) = 0.51245d0*surf_ten
                     x(l175-1+i) = -0.15966d0*surf_ten
                     x(l180-1+i) = -0.00019781d0*surf_ten
                     x(l185-1+i) = 0.00016392d0*surf_ten
                  else if (nbond == 3) then
                     x(l165-1+i) = 1.70d0 + 1.4d0
                     x(l170-1+i) = 0.070344d0*surf_ten_bb
                     x(l175-1+i) = -0.019015d0*surf_ten_bb
                     x(l180-1+i) = -0.000022009d0*surf_ten_bb
                     x(l185-1+i) = 0.000016875d0*surf_ten_bb
                  else
               ...
               if (atype == 'O ') then
                  ...
                  x(l170-1+i) = 0.68563d0*surf_ten_bb
                  x(l175-1+i) = -0.1868d0*surf_ten_bb
                  x(l180-1+i) = -0.00135573d0*surf_ten_bb
                  x(l185-1+i) = 0.00023743d0*surf_ten_bb
               else if (atype == 'O2') then
                  x(l165-1+i) = 1.60d0 + 1.4d0
                  x(l170-1+i) = 0.88857d0*surf_ten
                  x(l175-1+i) = -0.33421d0*surf_ten
                  x(l180-1+i) = -0.0018683d0*surf_ten
                  x(l185-1+i) = 0.00049372d0*surf_ten
               else
                  if (nbond == 1) then
                     x(l165-1+i) = 1.60d0 + 1.4d0
                     x(l170-1+i) = 0.77914d0*surf_ten
                     x(l175-1+i) = -0.25262d0*surf_ten
                     x(l180-1+i) = -0.0016056d0*surf_ten
                     x(l185-1+i) = 0.00035071d0*surf_ten
                  else if (nbond == 2) then
                     x(l165-1+i) = 1.60d0 + 1.4d0
                     x(l170-1+i) = 0.49392d0*surf_ten
                     x(l175-1+i) = -0.16038d0*surf_ten
                     x(l180-1+i) = -0.00015512d0*surf_ten
                     x(l185-1+i) = 0.00016453d0*surf_ten
                  else
               ...
               if (atype == 'N3') then
                  if (nbond == 1) then
                     ...
                     x(l170-1+i) = 0.078602d0*surf_ten
                     x(l175-1+i) = -0.29198d0*surf_ten
                     x(l180-1+i) = -0.0006537d0*surf_ten
                     x(l185-1+i) = 0.00036247d0*surf_ten
                  else if (nbond == 2) then
                     x(l165-1+i) = 1.65d0 + 1.4d0
                     x(l170-1+i) = 0.22599d0*surf_ten
                     x(l175-1+i) = -0.036648d0*surf_ten
                     x(l180-1+i) = -0.0012297d0*surf_ten
                     x(l185-1+i) = 0.000080038d0*surf_ten
                  else if (nbond == 3) then
                     x(l165-1+i) = 1.65d0 + 1.4d0
                     x(l170-1+i) = 0.051481d0*surf_ten
                     x(l175-1+i) = -0.012603d0*surf_ten
                     x(l180-1+i) = -0.00032006d0*surf_ten
                     x(l185-1+i) = 0.000024774d0*surf_ten
                  else
                  ...
                  if (nbond == 1) then
                     x(l165-1+i) = 1.65d0 + 1.4d0
                     x(l170-1+i) = 0.73511d0*surf_ten
                     x(l175-1+i) = -0.22116d0*surf_ten
                     x(l180-1+i) = -0.00089148d0*surf_ten
                     x(l185-1+i) = 0.0002523d0*surf_ten
                  else if (nbond == 2) then
                     x(l165-1+i) = 1.65d0 + 1.4d0
                     x(l170-1+i) = 0.41102d0*surf_ten_bb
                     x(l175-1+i) = -0.12254d0*surf_ten_bb
                     x(l180-1+i) = -0.000075448d0*surf_ten_bb
                     x(l185-1+i) = 0.00011804d0*surf_ten_bb
                  else if (nbond == 3) then
                     x(l165-1+i) = 1.65d0 + 1.4d0
                     x(l170-1+i) = 0.062577d0*surf_ten
                     x(l175-1+i) = -0.017874d0*surf_ten
                     x(l180-1+i) = -0.00008312d0*surf_ten
                     x(l185-1+i) = 0.000019849d0*surf_ten
                  else
               ...
               if (atype == 'SH') then
                  ...
                  x(l170-1+i) = 0.7722d0*surf_ten
                  x(l175-1+i) = -0.26393d0*surf_ten
                  x(l180-1+i) = 0.0010629d0*surf_ten
                  x(l185-1+i) = 0.0002179d0*surf_ten
               else
                  x(l165-1+i) = 1.90d0 + 1.4d0
                  x(l170-1+i) = 0.54581d0*surf_ten
                  x(l175-1+i) = -0.19477d0*surf_ten
                  x(l180-1+i) = -0.0012873d0*surf_ten
                  x(l185-1+i) = 0.00029247d0*surf_ten
               end if
            else if (atomicnumber .eq. 15) then
               if (nbond == 3) then
                  x(l165-1+i) = 1.90d0 + 1.4d0
                  x(l170-1+i) = 0.3865d0*surf_ten
                  x(l175-1+i) = -0.18249d0*surf_ten
                  x(l180-1+i) = -0.0036598d0*surf_ten
                  x(l185-1+i) = 0.0004264d0*surf_ten
               else if (nbond == 4) then
                  x(l165-1+i) = 1.90d0 + 1.4d0
                  x(l170-1+i) = 0.03873d0*surf_ten
                  x(l175-1+i) = -0.0089339d0*surf_ten
                  x(l180-1+i) = 0.0000083582d0*surf_ten
                  x(l185-1+i) = 0.0000030381d0*surf_ten
               else
               ...
               x(l170-1+i) = 0.68563d0*surf_ten
               x(l175-1+i) = -0.1868d0*surf_ten
               x(l180-1+i) = -0.00135573d0*surf_ten
               x(l185-1+i) = 0.00023743d0*surf_ten
            else
               ! write( 0,* ) 'bad atom type: ',atype
               ! call mexit( 6,1 )
               x(l165-1+i) = 1.70 + 1.4;
               x(l170-1+i) = 0.51245*surf_ten;
               x(l175-1+i) = -0.15966*surf_ten;
               x(l180-1+i) = -0.00019781*surf_ten;
               x(l185-1+i) = 0.00016392*surf_ten;
...
!GAMD not supported
   ...
   if (ntp > 0) then
      ...
      if(barostat /= 1 .and. baro_stochastic == 1) then
         write(6, '(/2x,a)') 'Stochastic correction to Berendsen (baro_stochastic is 1) can only be used with & 
                                               & Berendsen (barostat is 1)'
         DELAYED_ERROR
      end if
      if (baro_stochastic /= 0 .and. baro_stochastic /= 1) then
         write(6, '(/2x,a)') 'baro_stochastic must be 0 (Berendsen) or 1 (&
...
!   end if
   ...
   if (iextpot /= 0) then
        ...
        if (iextpot /= 1 .and. iextpot /= 2) then
          write(6, '(/,a)') 'The value of iextpot (in the &cntrl namelist) is invalid. Valid values are iextpot =' &
              //' 0 (No external potential),' &
              //' 1 (External potential added as a correction to the FF),' &
              //' and 2 (External potential overrides the FF)'
          DELAYED_ERROR
        end if

        if (extprog .eq. '') then
          write(6, '(/,a)') 'If iextpot is not 0 (in the &cntrl namelist), then extprog = "<EXTERNAL PROGRAM NAME>" must be set (in the &extpot namelist)'
          DELAYED_ERROR
        end if

#ifndef TORCHANI
        if (upcase(extprog) == 'TORCHANI') then
          write(6, '(/,a)') 'To use extprog = "TORCHANI" (in the &extpot namelist), Sander needs to be compiled with the TorchANI-Amber library enabled'
          DELAYED_ERROR
        end if
#else
        if (upcase(extprog) == 'TORCHANI' .and. (iextpot /= 2)) then
          write(6, '(/,a)') 'To use extprog = "TORCHANI" (in the &extpot namelist), iextpot = 2 (in the &cntrl namelist) is required'
          DELAYED_ERROR
        end if
#endif
...
!          DELAYED_ERROR
   ...
   if( (ntt/=4 .and. ipimd==2) .and. (ischeme/=1 .and. ipimd==2)) then
      write(6,*) 'NMPIMD requires Nose-Hoover chains (ntt=4) or middle scheme'
      inerr=1
   endif
...
#  endif
   ...
   if( len_trim(noshakemask) > 0 ) then
      ...
      call setnoshake(ix,noshakegp,ntc,num_noshake,.true.)
      if( ntf > 1 ) then
   ...
   do i=1,nbonh + nbona + nbper
      ...
      !  consistent on the two processors:
         ...
         if( ntc == 2 .and. ifsc == 0) then
            ...
            ! The test below is for qm/mm -> qm/mm transformations.
            ! Do not apply the test for mm -> qm/mm transformations.
            ! We need to know if the partner is qm/mm or mm
            call mpi_sendrecv( qmmm_nml%ifqnt, 1, MPI_LOGICAL, partner, 5, &
                               ifqnt_c, 1, MPI_LOGICAL, partner, 5, &
                               commmaster, ist, ierr )

            
            if ( (qmmm_nml%ifqnt .and. ifqnt_c) .and. &
                 & qmmm_nml%qmshake == 0) then
