...
subroutine bond(nbin,ib,jb,icb,x,f,eb)
...
subroutine angl(nbain,it,jt,kt,ict,x,f,eba)
...
#  include "flocntrl.h"
   ...
   integer nba,maxlen,i3,j3,k3,ic,jn,ist
...
subroutine ephi(nphiin,ip,jp,kp,lp,icp,cg,iac,x,f,dvdl, &
...
#  include "extra_pts.h"
   ...
   _REAL_ cgi,cgj
   integer ia1,ia2,ibig,isml,ii,jj,kk,ll
   integer ic,inc, ic0
...
!    igbNEzero: if( igb /= 0 .or. ipb /= 0 ) then
    igbNEzero: if( igb /= 0 .or. ipb /= 0 .or. ipgm /= 0 ) then
#endif
...
! Note: this routine modeled on xconst() and is very similar
   ...
   ! nattgtrms from "../include/memory.h"
   ! ntr from "../include/md.h"
#  include "../include/memory.h"
#  include "../include/md.h"
#  include "extra.h"
#ifdef MPI
...
! atoms but rmsd value is calculated for 'nattgtrms' atoms
   implicit none
   ! natom and nattgtfit,nattgtrms are from "../include/memory.h"
   ! ntr is from "../include/md.h"
#  include "../include/memory.h"
#  include "../include/md.h"
#  include "flocntrl.h"
#ifdef MPI
...
#  include "extra_pts.h"
   ...
   _REAL_ cosnp,sinnp
   _REAL_ z10,z20,z12,ap0,ap1,ct0,ct1,s,ftem
   _REAL_ epl
   integer ii,jj,kk,ll
   integer ic,inc
   integer i3,j3,k3,l3,k3t,l3t
