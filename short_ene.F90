...
!------------------------------------------------------------------------------
#ifdef HAS_10_12
subroutine get_nb_energy(iac, ico, ntypes, charge, cn1, cn2, cn6, cn7, cn8, C4Pairwise, force, &
                         numatoms, ipairs, ewaldcof, eedtbdns, eed_cub, & !New2021
...
#include "def_time.h"
  ...
  integer numatoms, maxnblst, mpoltype, C4Pairwise
  integer iac(*), ico(*), ntypes, ee_type, eedmeth, cn7(*)
  _REAL_ charge(*), cn1(*), cn2(*), cn6(*), cn8(*) !New2021
#ifdef HAS_10_12
  _REAL_ asol(*), bsol(*)
#endif
        ...
        if (ntot > 0) then
          if (mpoltype == 0) then
            ...
            call short_ene(i, xk, yk, zk, ipairs(numpack), ntot, nvdw, nhbnd, &
                           ...
                           ico, cn1, cn2, cn6, cn7, cn8, C4Pairwise, filter_cut, eelt, evdw, force, &
                           dir_vir, ee_type, eedmeth, dxdr, eedvir, & !New2021
            ...
            call short_ene(i, xk, yk, zk, ipairs(numpack), ntot, nvdw, nhbnd, &
                           ...
                           r_stack(l_real_y), r_stack(l_real_z), &
                           r_stack(l_real_r2), i_stack(l_int), cn3, cn4, cn5)
            !write(6,*) r_stack(l_real_r2) !New2021
            !write(6,*) ipairs(numpack)
#endif
            ...
            call short_ene_dip(i, xk, yk, zk, ipairs(numpack), ntot, nvdw, &
                               ...
                               dipole, ntypes, iac, ico, cn1, cn2, cn6, cn7, cn8, C4Pairwise, & 
                               filter_cut, eelt, epol, evdw, ehb, force, & !New2021
...
subroutine short_ene(i, xk, yk, zk, ipairs, ntot, nvdw, nhbnd, eedtbdns, &
                     ...
                     cn6, cn7, cn8, C4Pairwise, filter_cut, eelt, evdw, force, dir_vir, ee_type, &
                     eedmeth, dxdr, eedvir, cache_df, cache_x, cache_y, & !New2021
...
subroutine short_ene(i, xk, yk, zk, ipairs, ntot, nvdw, nhbnd, eedtbdns, &
  ...
  integer ipairs(*), ee_type, eedmeth, C4Pairwise !New2021
  _REAL_ eed_cub(*), eed_lin(2,*), charge(*), dir_vir(3,3), eedvir
  _REAL_ eedtbdns, filter_cut, dxdr
  integer ntypes, iac(*), ico(*), cn7(*)
  _REAL_ cn1(*), cn2(*), cn6(*), cn8(*), eelt, evdw, force(3,*) ! New2021
...
#include "../include/md.h"
  ...
  integer im_c4, im_new, icount !New2021
...
#if 0  /* following seems to give the correct answers, but greatly slows things down */
!$omp parallel do &
!$omp& private(j,delrinv,x,ind,dx,e3dx,e4dx,switch,d_switch_dx,b0,b1,cgj,comm1,ecur,dfee,delr2inv) &
!$omp&    reduction(+:ee_vir_iso,eelt)
#endif
    do im_new = 1, icount
...
#     include "ew_directp.h"
    ...
    do im_new = 1, icount
      ...
      ! Don't use a switch: Shifted  Coulomb
      switch = (1.0 - delr2/cut2)**2
      d_switch_dx = 4.0*delr*(delr2-cut2)/cut4
      b0 = switch*delrinv
      b1 = b0 - d_switch_dx
      cgj = charge(j)
#ifdef LES
...
#     include "ew_directp.h"
    ...
    do im_new = 1, icount
      ...
      if (nsc(i) == nsc(j) .and. emil_sc .eq. 0) then          
        ...
        switch_c = 1.0d0 - switch 
        eelt = eelt - comm1 * delrinv * switch_c
        dfee = - comm1 * (switch_c * delrinv + d_switch_dx) * delr2inv
      ...
      !  delr = 1.0d0 / delrinv
        ...
        if (isProcessV1) then
          ...
          sc_dvdl_ee = sc_dvdl_ee + &
                       switch / sceeorder * comm1 * denom_n * scbeta
          dfee = -(d_switch_dx * comm1 * denom * delrinv) + &
                  (switch * comm1 * denom_n * delr_n * delr2inv)
      if (ifcr .ne. 0) then
            call cr_add_dcdr_factor(i, b0*cgj)
            call cr_add_dcdr_factor(j, b0*cgi)
...
subroutine short_ene_dip(i, xk, yk, zk, ipairs, numtot, numvdw, ewaldcof, &
                         ...
                         iac, ico, cn1, cn2, cn6, cn7, cn8, C4Pairwise, filter_cut, eelt, epol, &
                         evdw, ehb, frc, field, pol, pol2, dir_vir, ee_type, & !New2021
...
#include "../include/md.h"
  ...
  integer ipairs(*), ee_type, eedmeth, mpoltype, C4Pairwise !New2021
  _REAL_ ewaldcof, eed_cub(4,*), eed_lin(2,*), charge(*), dipole(3,*), &
         dir_vir(3,3), eedvir
  _REAL_ eedtbdns, filter_cut, dxdr
  integer ntypes,iac(*),ico(*), cn7(*)
  _REAL_ cn1(*), cn2(*), cn6(*), cn8(*), eelt, epol, evdw, ehb, frc(3,*), &
         field(3,*), pol(*) ! New2021
  _REAL_ pol2(*)
  _REAL_ cn3(*), cn4(*), cn5(*)
#ifdef HAS_10_12
  ...
  do m = 1, numvdw
    ...
    if (delr2 < filter_cut2) then
      ...
      if  (lj1264 == 1 .and. plj1264 == 1) then !New2021
         mr4 = delr2inv*delr2inv
         f4 = cn6(ic)*mr4
         do im_c4 = 1, C4Pairwise
            if(MIN(i,j) == (MIN(cn7(im_c4*3-2), cn7(im_c4*3-1))+3)/3 & 
            .and. MAX(i,j) == (MAX(cn7(im_c4*3-2), cn7(im_c4*3-1))+3)/3) then
               f4 = f4 + cn8(im_c4)*mr4
            end if
         end do
         evdw = evdw + f12 - f6 - f4
      else if (lj1264 == 1 .and. plj1264 /= 1) then
         mr4 = delr2inv*delr2inv
         f4 = cn6(ic)*mr4
         evdw = evdw + f12 - f6 - f4
      else if (lj1264 /= 1 .and. plj1264 == 1) then
