...
! ZH: For pGM: keep track of the numbers of full and short scf pairs
integer, save :: list_short_tot
integer, save :: list_tot
...
#  endif /* MPI */
  ...
  listtot = numpack
  ! For pGM: save the number of full scf pairs
  list_tot = numpack
  return
...
subroutine pack_nb_list(kk, i, xk, yk, zk, imagcrds, cutoffsq, numlist, &
  ...
  use pol_gauss_mdin, only : ipgm, scf_local_cut
...
#  include "parallel.h"
  ...
  ! Local variables for pGM
  ! lex: excluded pairs
  ! scf_local_cut_sq: short scf pair cutoff squared
  integer lex
  _REAL_ scf_local_cut_sq

  scf_local_cut_sq = (scf_local_cut + skinnb) * (scf_local_cut + skinnb)
...
# endif /* MPI, SOFTCORE */
  ...
  do m = kk+1, numlist
    ...
    k = bckptr(n)
    deadik = ((ibelly(k) == 0) .and. deadi)
    ! For pGM: The following is to include excluded atoms
    ! if ((exclude(k) /= i) .and. .not. deadik) then
    if (.not. deadik) then
    if (exclude(k) /= i) then ! NOT excluded
      ...
      r2 = dx*dx + dy*dy + dz*dz
      if (r2 < cutoffsq) then
        ! For pGM: Count short scf pairs
        if (r2 < scf_local_cut_sq) then
          list_short_tot = list_short_tot + 1
        end if
#  ifdef TEST_CLOSEATOMS
        if (r2 < .5) then
          write(6,190) i, k, r2
...
#  endif
    ...
    ! For pGM: Collect excluded atoms
    else if (ipgm >= 1) then ! YES excluded
      mygrdlist(n) = ior(mygrdlist(n), 1)
      num = num + 1
      lex = lex + 1
      iwe(lex) = ior(n, ishft(itran(m), 27)) ! bitwise optimization
      ! For pGM: Apparently these are part of the short scf pairs
      dx = imagcrds(1,n) - xk + x_tran
      dy = imagcrds(2,n) - yk + y_tran
      dz = imagcrds(3,n) - zk + z_tran
      r2 = dx*dx + dy*dy + dz*dz
...
# endif 
  ...
  npr = lpr + lhb
  ! For pGM: Pack excluded atoms into iwa for electrostatics
  if (ipgm >= 1) then
  do k = 1, lex
    iwa(npr+k) = iwe(k)
  end do
  ...
  numvdw(i) = lpr
  numhbnd(i) = lhb
  ! For pGM: Set up excluded atom pointer
  if (ipgm >= 1) numexc(i) = lex
  do k = 1, npr
    numpack = numpack + 1
...
#include "box.h"
  ...
  if (periodic == 0) then
    ...
    if (fracmax > one .or. fracmin < zero) then
       ! recalculate the virtual box
       !write(6,'(3F13.6)')ucell(1,1),ucell(2,2),ucell(3,3)
       !write(6,'(3F13.6)')recip(1,1),recip(2,2),recip(3,3)
       !write(6,'(3F13.6)')xbox0,ybox0,zbox0
       call firstbox_from_crd(natom,crd)
       !write(6,'(3F13.6)')ucell(1,1),ucell(2,2),ucell(3,3)
       !write(6,'(3F13.6)')recip(1,1),recip(2,2),recip(3,3)
       !write(6,'(3F13.6)')xbox0,ybox0,zbox0
       ...
       ! If it is still a problem, then print the error message
       if (fracmax > one .or. fracmin < zero) then
          boxerr = .true.
       end if
    end if
