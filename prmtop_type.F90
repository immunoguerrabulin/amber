...
module prmtop_type
   ...
   type :: prmtop_struct
      ...
      integer :: nmxrs, ifcap
      integer :: numextra, ncopy, C4Pairwise !New2021
...
!     New2021
      integer, allocatable, dimension(:)   :: lennard_jones_dcoef
...
!     New2021
      double precision, allocatable, dimension(:) :: lennard_jones_dvalue
...
!     call destroy_prmtop_struct(parm)
      ...
      SAFE_READ &
         ...
         parm%numextra, parm%ncopy, parm%C4Pairwise ! New2021
...
!     New2021
      call nxtsec(LUN, 6, 1, rfmt, 'LENNARD_JONES_DCOEF', fmt, iok)
      if (iok == 0) then
         allocate(parm%lennard_jones_dcoef(3*parm%C4Pairwise))
         SAFE_READ (parm%lennard_jones_dcoef(i), i=1, 3*parm%C4Pairwise)
      end if
      call nxtsec(LUN, 6, 1, rfmt, 'LENNARD_JONES_DVALUE', fmt, iok)
      if (iok == 0) then
         allocate(parm%lennard_jones_dvalue(parm%C4Pairwise))
         SAFE_READ (parm%lennard_jones_dvalue(i), i=1, parm%C4Pairwise)
      end if
...
!     New2021
      SAFE_DEALLOC(lennard_jones_dcoef)
