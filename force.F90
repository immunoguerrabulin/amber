...
#ifdef DPRC
  use dprcmod, only : dprc_nml, cpt_dprc_iface
#endif
...
#include "flocntrl.h"
  ...
  !JOSE MOD: Cached FIRES force for multiple time step reuse
  logical, save :: fires_cache_init = .false.
  _REAL_, allocatable, save :: f_fires_cache(:)
  _REAL_, allocatable, save :: f_snapshot(:)
  _REAL_, save :: efires_cache
  integer, save :: last_fires_step
    ...
    if (iextpot == 2) then
        if (pme_external_only_overrides_bonded) then
            if (pme_external_needs_neighborlist) then
                ! Force a full rebuild if both overrides bonded + needs nblist
                ! since in this case nblist has to be called twice per force() call
                qsetup = .true.
            endif
            call nonbond_list( &
                x, ix(i04), ix(i06), ix(i08), &
                ix(i10), ntypes, &
  ...
  if (igb == 0 .and. ipb == 0 .and. iyammp == 0) then
    ...
    if (iamoeba == 1) then
      call AM_NonBond_eval(natom, x, f, vir, xx, ipairs, evdw, eelt, epolar, &
       ...
       call pGM_NonBond_eval(natom, x, f, vir, ix, xx, nspm, ix(i70), xx(lmass), ipairs, &
                             ntypes, ix(i04), ix(i06), cn1, cn2, &
                             evdw, eelt, &
                             diprms, dipiter)
    else if (iextpot == 2) then
        ! VDW 1-4 and electro 1-4
        ee14 = 0.0d0
        enb14 = 0.0d0
        epol = 0.0d0
        esurf = 0.0d0
        ...
        if (pme_external_only_overrides_bonded) then
            call ewald_force(x, natom, ix(i04), ix(i06), xx(l15), cn1, cn2, &
                             cn6, cn7, cn8, C4Pairwise, eelt, epolar, &
                             f, xx, ix, ipairs, xx(l45), & ! New2021
                             virvsene, xx(lpol), &
#ifdef HAS_10_12
            ...
            ! All Sander's energies are overriden by an external pot
            ! zero all non-bond energies
            ! VDW
            evdw = 0.0d0
            ! Electro
            eelt = 0.0d0
    ...
    call timer_stop(TIME_EWALD)
#ifdef MPI
      if (mytaskid == 0 .or. ipgm /= 0) then ! RL: pGM energy terms have not been reduced 
#endif
        pot%vdw   = evdw
        pot%elec  = eelt
   ...
   ! ASM
#ifdef MPI
   if (sanderrank == 0) call asm_calc(x(1:natom*3), f(1:natom*3), asm_energy)
#endif
...
#  endif
  ...
  !  RL: pGM vir(1:3) has been correctly assigned and will be reduced 
  !  along with pot and frc by calling fdist() below
  if (mytaskid == 0 .and. iamoeba == 0 .and. ipgm == 0) then
...
#  endif
  ...
  if ( dprc_nml%idprc > 0 ) then
     call cpt_dprc_iface(x,f,pot%ml)
  end if
#endif
  ...
  if (iextpot .gt. 0) then
      if (igb == 0) then
         if (pme_external_needs_neighborlist) then
            ...
            ! This always forces a rebuild due to qsetup=.true.
            call nonbond_list( &
                x, ix(i04), ix(i06), pme_external_dummyiblo, &
                ix(i10), ntypes, &
                natom/am_nbead, xx, ix, ipairs, ntnb, ix(ibellygp), &
                belly, newbalance, qsetup, do_list_update &
            )
...
!  endif
  ...
  pot%constraint = pot%constraint + eshf + epcshf + pot%noe + &
                   sum(enmr(1:6)) + ealign + ecsa + pot%emap + enfe + efires
...
#endif
  ! Add FIRES energy to the restraint bucket in non-MTS runs only (handled here)
  if (fires == 1 .and. ipimd == 0) then
    if (.not. (mts_n > 1 .and. mts_fires > 0.0d0)) then
  ...
  ! ASM
#ifdef MPI
  pot%tot = pot%tot + asm_energy
#endif

  ! machine learning correction
  pot%tot = pot%tot + pot%ml
