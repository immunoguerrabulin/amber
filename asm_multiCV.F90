...
#include "../include/dprec.fh"    
    ...
    use asm_CV
    use asm_utilities, only : next_unit
    ...
    _REAL_, dimension(12) :: centers, centers_gradient
...
contains
    ...
    function get_center(indexes, x) result(center)
        ...
        _REAL_, dimension(:), intent(in) :: x
        _REAL_, dimension(3) :: center, reference, xyz
    ...
    function unpack_gradient(natoms, indexes, center_gradient) result(gradient)
        ...
        integer, dimension(:), intent(in) :: indexes
        _REAL_, dimension(:), intent(in) :: center_gradient

        integer :: i
        _REAL_, dimension(natoms*3) :: gradient
    ...
    subroutine set_xyz(index, x, xyz)
        integer, intent(in) :: index
        _REAL_, dimension(:), intent(inout) :: x
        _REAL_, dimension(3), intent(in) :: xyz

        x(index*3-2:index*3) = xyz

    end subroutine set_xyz

end module asm_multiCV
