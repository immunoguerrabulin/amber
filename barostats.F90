...
module barostats
   ...
   integer, save      :: mcbar_call = 0 ! ZH/UCI: keep track of mctrial call to force()
...
!                     not to change the size of the volume move
   ...
   public mcbar_trial, mcbar_setup, mcbar_summary, mcbar_call
...
! Local parameters
   ...
   ! ZH/UCI: we are starting the call to force().
   mcbar_call = 1
   ...
   call force(xx, ix, ih, ipairs, x, f, new_ener, vir,  &
      fs, rborn, reff, onereff, qsetup, do_list_update, nstep)
   ! ZH/UCI: we have finished the call to force()
   mcbar_call = 0
