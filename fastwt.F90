...
subroutine setlep (iwatat, natom, x0, x1, mxatm, iorwat, rbtarg)
   !*****************************************************************
   !                                                               **
   !    Subroutine : setlep - reset positions of rigid waters      **
...
!+ determines how to call setlev(), the 3-point water shake routine
subroutine quick3v(x1, v1, ifstwr, natom, nres, ipres)
   ...
   ! INPUT:
   ! X1(I): constrained position array (input)
   ! V1(I): constrained velocity array (input/output)
...
#  include "../include/md.h"
   ...
   if (ienwat > 0) then
      ...
      ! write(0,*) 'calling setlev',mytaskid,istart,iend,ibgwat,ienwat
      ...
      call setlev (istart,iend,x1,v1,natom,iorwat,rbtarg)
...
subroutine setlev (iwatat, natom, x1, v1, mxatm, iorwat, rbtarg)
   ...
   !*****************************************************************
   !                                                               **
   !    Subroutine : setlev - reset velocities of rigid waters     **
   !    Author : Shuichi Miyamoto                                  **
   ...
   integer iwatat, natom, mxatm, iorwat
   _REAL_  x1, v1, rbtarg
   dimension  x1(3,mxatm),v1(3,mxatm),rbtarg(9)
   !*****************************************************************
   ! Most of the variable names correspond to those in the reference.
   
   !     iwatat : pointer for the first atom of waters
   !     natom  : No. of total atoms in system
   !     x1(3,i): position after applying constraints (input)
   !     v1(3,i): velocity before applying constraints (input)
   ...
   !*****************************************************************
   _REAL_  wh, wo, woh, whh, woh2, wowh2, whwh
   ...
   do i = iwatat, natom, 3
      ...
      xab = x1(1,i+ind2) - x1(1,i+ind1)
      yab = x1(2,i+ind2) - x1(2,i+ind1)
      zab = x1(3,i+ind2) - x1(3,i+ind1)
      xbc = x1(1,i+ind3) - x1(1,i+ind2)
      ybc = x1(2,i+ind3) - x1(2,i+ind2)
      zbc = x1(3,i+ind3) - x1(3,i+ind2)
      xca = x1(1,i+ind1) - x1(1,i+ind3)
      yca = x1(2,i+ind1) - x1(2,i+ind3)
      zca = x1(3,i+ind1) - x1(3,i+ind3)
      xvab = v1(1,i+ind2) - v1(1,i+ind1)
      yvab = v1(2,i+ind2) - v1(2,i+ind1)
      zvab = v1(3,i+ind2) - v1(3,i+ind1)
      xvbc = v1(1,i+ind3) - v1(1,i+ind2)
      yvbc = v1(2,i+ind3) - v1(2,i+ind2)
      zvbc = v1(3,i+ind3) - v1(3,i+ind2)
      xvca = v1(1,i+ind1) - v1(1,i+ind3)
      yvca = v1(2,i+ind1) - v1(2,i+ind3)
      zvca = v1(3,i+ind1) - v1(3,i+ind3)
      ...
      v1(1,i+ind1) = v1(1,i+ind1) + ( xeab * tabd - xeca * tcad ) * wh / deno
      v1(2,i+ind1) = v1(2,i+ind1) + ( yeab * tabd - yeca * tcad ) * wh / deno
      v1(3,i+ind1) = v1(3,i+ind1) + ( zeab * tabd - zeca * tcad ) * wh / deno
      v1(1,i+ind2) = v1(1,i+ind2) + ( xebc * tbcd - xeab * tabd * wo ) / deno
      v1(2,i+ind2) = v1(2,i+ind2) + ( yebc * tbcd - yeab * tabd * wo ) / deno
      v1(3,i+ind2) = v1(3,i+ind2) + ( zebc * tbcd - zeab * tabd * wo ) / deno
      v1(1,i+ind3) = v1(1,i+ind3) + ( xeca * tcad * wo - xebc * tbcd ) / deno
      v1(2,i+ind3) = v1(2,i+ind3) + ( yeca * tcad * wo - yebc * tbcd ) / deno
      v1(3,i+ind3) = v1(3,i+ind3) + ( zeca * tcad * wo - zebc * tbcd ) / deno
...
subroutine getwds(igraph    ,nres      ,ipres     ,lbres     , &
   ...
   ! derives from them the parameters required by fast 3-point water routine
   ! SETLEP.
   ...
   nres_loop: do i = 1,nres
      ...
      do  j = 1,3
         ...
         if (igraph(iat) == iowtnm) then
            ...
            if( imin .ne. 0 .and. ntmin .eq. 3 ) rbtarg(6) = 1.d0
         else if (igraph(iat) == ihwtnm(1)) then
            ihf1 = 3*(iat-1)
            rbtarg(7) = 1.0d0/winv(iat)
            if( imin .ne. 0 .and. ntmin .eq. 3 ) rbtarg(7) = 1.d0
         else if (igraph(iat) == ihwtnm(2)) then
            ihf2 = 3*(iat-1)
            rbtarg(7) = 1.0d0/winv(iat)
            if( imin .ne. 0 .and. ntmin .eq. 3 ) rbtarg(7) = 1.d0
...
subroutine b4setl(rhh,roh,rbtarg)
   ...
   ! The calculated values are returned in RBTARG(1-5):
