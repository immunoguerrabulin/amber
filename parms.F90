...
integer, allocatable, dimension(:) :: cn7       ! C4Pairwise New2021
_REAL_, allocatable, dimension(:) :: cn8        ! C4Value New2021
...
subroutine allocate_parms
   ...
   allocate(rk(numbnd), req(numbnd),             & ! bond params
            ...
            cn7(3*C4Pairwise), cn8(C4Pairwise), solty(natyp),        & ! New2021 unused ??
...
subroutine clean_parms
   ...
   if (allocated(cn7))     deallocate(cn7) !New2021
   if (allocated(cn8))     deallocate(cn8) !New2021
...
#  include "parallel.h"
   ...
   call mpi_bcast(C4Pairwise, 1, MPI_INTEGER, 0, commsander, ierror) !New2021
   ...
   call mpi_bcast(cn8, C4Pairwise, MPI_DOUBLE_PRECISION, 0, commsander, ierror) !New2021

   ! Broadcast the integer array
   call mpi_bcast(ipn, nptra, MPI_INTEGER, 0, commsander, ierror)
   call mpi_bcast(cn7, 3*C4Pairwise, MPI_INTEGER, 0, commsander, ierror) !New2021
