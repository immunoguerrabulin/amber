...
module bintraj
   ...
   integer :: remd_valuesVarID, remd_typesVarID, Cell_lengthVarID
   integer :: Cell_angleVarID, VelocVarID, vel_TimeVarID, vel_frame
   ...
   if (ntwx.gt.0) then
      ...
      if (facc == 'W' .or. .not.crd_file_exists) then
         ...
         if ( NC_create( mdcrd, owrite, .false., atomCnt, &
                         ...
                         remd_valuesVarID, frameDID=FrameDimID ) ) then
         ...
         if (NC_setupMdcrd(crd_ncid, title, crd_frame, ncatom, &
                           ...
                           Cell_lengthVarID, Cell_angleVarID, remd_valuesVarID)) call mexit(6,1)
         ...
         if ( (remd_valuesVarID.ne.-1).neqv.isRemd ) then
            write(6,'(a)') 'Error: Cannot append to netcdf traj, remd_values info mismatch.'
            call mexit(6,1)
         endif
...
#  ifdef MPI
   if ( ioutfm.ne.0 ) then
      if ( NC_defineRemdIndices(crd_ncid, remd_dimension, remd_indices_var_id, &
                                repidxVID, crdidxVID, &
                                remd_types, .false., (rem.ne.0), (rem.eq.-1), &
                                frameDID=FrameDimID, &
                                remd_valuesVID=remd_valuesVarID, &
                                remd_typesVID=remd_typesVarID) ) &
         call mexit(6,1)
   endif
#  endif
#else
...
#  ifdef MPI
      use remd, only: rem, my_remd_data, replica_indexes, remd_dimension, &
                      stagid, remd_repidx, remd_crdidx, remd_types
...
#  ifdef MPI
   _REAL_, dimension(remd_dimension) :: remd_values
   integer :: i
#  endif
      ...
      case (MDCRD_UNIT)
         if (n == 3 .and. ntb > 0) then       ! Assume this is box (fails on one atom systems)
            ...
            call checkNCerror(nf90_put_var(crd_ncid,CoordVarID, r(istart:n), &
                              ...
                              'write atom coords')
#  ifdef MPI
            ! If this is a replica run write temp0, solvph, and/or solve
            if (rem.ne.0) then
               ...
               if (rem .eq. -1) then
                  ...
                  do i = 1, remd_dimension
                    if (remd_types(i) == 1) then
                      remd_values(i) = my_remd_data%mytargettemp
                    else if (remd_types(i) == 3) then
                      remd_values(i) = replica_indexes(i)
                    else if (remd_types(i) == 4) then
                      remd_values(i) = target_ph
                    else if (remd_types(i) == 5) then
                      remd_values(i) = target_e
                    end if
                  end do
                  call checkNCerror(nf90_put_var(crd_ncid, remd_valuesVarID, &
                                    remd_values(:), &
                                    start = (/ 1, crd_frame /), &
                                    count = (/ remd_dimension, 1 /)), &
                                    'write remd_values')
               else if (rem==4) then
                  call checkNCerror(nf90_put_var(crd_ncid,remd_valuesVarID,target_ph, &
                                                 start = (/ crd_frame /) ), &
                                    'write replica pH')
               else if (rem==5) then
                  call checkNCerror(nf90_put_var(crd_ncid,remd_valuesVarID,target_e, &
                                                 start = (/ crd_frame /) ), &
