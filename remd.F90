   ...
   if (rem == 3) then
      ...
      if (master) then
         ...
         if (isgld > 0) then
            ...
            ! build RXSGLD momentum guiding factor table
           ...
           if (stagid == 0) then
             !stagid = tempsglookup(numreps, temp0, tempsg, &
             !  sgfti, sgffi, statetable, tempsgtable, sgfttable, sgfftable)
             stagid = tempsglookup(numreps, tempsg,  tempsgtable)
           else
             !temp0 = statetable(stagid)
             tempsg = tempsgtable(stagid)
           ...
           if(tempsgtable(1) /= statetable(1) .or. sgfttable(1) /= 0.0d0 .or. sgfftable(1) /= 0.0d0) then
                  write (6,*) "================================"
                  write (6,*) "No guiding effect allowed on the first replica!"
                  write (6,*) "  temp, tempsg, sgft, sgff=: ", statetable(1), tempsgtable(1), sgfttable(1), sgfftable(1)
                  write (6,*) "================================"
                  call mexit(6,1)
           end if
...
#  endif
   ...
   ! RXSGLD
   if (allocated(tempsgtable)) deallocate(tempsgtable)
...
! CARLOS: NEED BARRIER?
      ...
      if(my_remd_data%myscaling < 0.0d0) then
         ...
         if (isgld > 0) then
            ...
            psgldg=psgldgtable(stagid)
            tsgset=o_temp0
            !o_stagid = tempsglookup(remd_size, tsgset, tempsg,sgfti, sgffi, &
            !                        statetable, tempsgtable, sgfttable, sgfftable)
