...
contains
	...
	_REAL_ function spline_value( x, coef )
	
		_REAL_, intent(in) :: x
		_REAL_, dimension(:,:),intent(in) :: coef
		
		integer :: idx
		_REAL_ :: x_max, y_max, k
	...
	_REAL_ function spline_min(coef)
	...
	_REAL_ function spline_max(coef)
	...
	_REAL_ function spline_der( x, coef ) !Derivative dy(x)/dx
	...
	_REAL_ function spline_integrate( x_min, x_max, coef )
	
		_REAL_, intent(in) :: x_min, x_max
		_REAL_, dimension(:,:),intent(in) :: coef
	...
	subroutine CubicSplines( x, y, coef )
		...
		_REAL_, dimension(size(x)-1,5), intent(out) :: coef

		integer :: i, n
	
		_REAL_, dimension(size(x)-1) :: h
		_REAL_, dimension(size(x)-2) :: d, b
		_REAL_, dimension(size(x)) :: s
	...
	subroutine CubicSplinesND( x, y, coef )
		...
		_REAL_, dimension(:,:), intent(in) :: y
		_REAL_, dimension(size(x)-1,5,size(y, dim=1)), intent(out) :: coef
	...
	function spline_value_ND(x, coef)
		...
		_REAL_, intent(in) :: x
		_REAL_, dimension(:,:,:), intent(in) :: coef
		_REAL_, dimension(size(coef, dim=3)) :: spline_value_ND
	...
	_REAL_ function spline_grad_mod_ND(x, coef)
	...
	function spline_der_ND(x, coef)
		...
		_REAL_, intent(in) :: x
		_REAL_, dimension(:,:,:), intent(in) :: coef
		_REAL_, dimension(size(coef, dim=3)) :: spline_der_ND
	...
	subroutine CubicSplinesFit( a, b, coef )
		...
		_REAL_, dimension(:), intent(in) :: a, b	  !Vectors with coordinates of data points
		_REAL_, dimension(:,:), intent(out) :: coef !coef(:,1) - x_i values, coef(:,2:5) - spline coefficients
		
		integer :: n, np	!number of data points
		integer :: i, j, idx
		_REAL_ :: h		! x_i+1 - x_i
		_REAL_, dimension(size(coef, dim=1)+1) :: x, y	!Coordinates of spline reference points
		integer, dimension(size(a)) :: aidx	!indexes of splines to which each data point belongs
		_REAL_, dimension(size(coef, dim=1)+1) :: Mval !Final M values
		_REAL_, dimension(size(coef, dim=1)+1) :: cpb
		_REAL_, dimension(size(coef, dim=1)+1,size(coef, dim=1)+1) :: M  !m = My
		_REAL_, dimension(size(a),size(coef, dim=1)+1) :: C !C_ji = d/dy_i S(a_j)
		_REAL_, dimension(size(coef, dim=1)+1,size(a)) :: CT
		_REAL_, dimension(size(coef, dim=1)+1,size(coef, dim=1)+1) :: Cp !Coefficient matrix for the final equation system
...
#include "../include/dprec.fh"	
		_REAL_, dimension(n,n), intent(in) :: A
		_REAL_, dimension(n), intent(in) :: b
		_REAL_, dimension(n), intent(out) :: x
		
		integer :: info
		integer, dimension(n) :: ipiv
		_REAL_, dimension(n,n) :: Mtmp
	...
	function SplineMatrix( n, h ) 
		...
		integer, intent(in) :: n
		_REAL_, intent(in) :: h
		_REAL_, dimension(n,n) :: SplineMatrix
		_REAL_, dimension(n-2,n-2) :: X
		integer, dimension(n-2,n) :: Y
		
		integer :: i, j
		_REAL_, parameter  :: lambda = 1.31695789692482_8
	...
	!WARNING! d and b values are not conserved!
		...
		integer, intent(in) :: n
		_REAL_, dimension(:), intent(inout) :: d, subd, supd, b
		_REAL_, dimension(n) :: Thomas
		_REAL_, dimension(:), allocatable :: c
...
end module asm_splines_utilities
