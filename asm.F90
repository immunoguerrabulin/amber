...
module asm_mod
    ...
    use molecule
    use asm_CV_utilities
    use asm_utilities
    use asm_splines_utilities
...
#include "parallel.h"
    ...
    public :: asm_define, asm_calc, asm, minimize
    ...
    _REAL_  :: force_scale !For gradual increase of forces during preparation
    _REAL_  :: gamma, position_gamma, force_gamma, force_kappa !friction
    _REAL_  :: Mav_damp !damping coefficient for Mav averaging
    _REAL_  :: RT !R*temperature
    _REAL_  :: K_d !Force constant for the d coordinate
    _REAL_  :: string_length !Arc-length of the string (using cubic splines)
    _REAL_  :: dpos, dK !displacement of the node position and the force constant
    _REAL_  :: mean_dx, mean_sigma2 !DEBUG
    ...
    _REAL_, dimension(:), allocatable :: K_l, pos !Force constants and positions
    _REAL_, dimension(:), allocatable :: weight_buffer !Buffer with weights for reweighting to path CV
    _REAL_, dimension(:,:), allocatable :: string !Current string
    _REAL_, dimension(:,:), allocatable :: CV_buffer !Buffer for sampled points in CV space
    _REAL_, dimension(:,:), allocatable :: n !n vectors (tangents to the string)
    _REAL_, dimension(:,:), allocatable :: decomp_coef !coefficients for PMF decomposition
    _REAL_, dimension(:,:), allocatable :: CV !Current values of CVs at all nodes
    _REAL_, dimension(:), allocatable :: dz !Displacement vector for the string node
    ...
    _REAL_, dimension(:,:), allocatable :: Mav, Minv, B !Average M, inv(Mav), biasing matrices
    
    _REAL_, dimension(:,:,:), allocatable :: string_spline !Cubic splines interpolation of the string
    _REAL_, dimension(:,:,:), allocatable :: decomp_coef_spline !Cubic splines interpolation for PMF decomposition
    _REAL_, dimension(:,:,:), allocatable :: string_spline_smooth !Same, but for smooth normal vectors
    ...
    _REAL_  :: lambda !Inverse distance between two adjacent nodes
    logical :: z_bias !Whether the bias on the orthogonal direction should be used for path CV
    logical :: reweight_z
    ...
    _REAL_, dimension(:), allocatable :: arc !arc length upto each node (for path CV)
    _REAL_, dimension(:), allocatable :: force, PMF !mean force and PMF along the path (using path CV)
    _REAL_, dimension(:,:), allocatable :: path !set of equidistant points along the string (with extrapolation)
    _REAL_, dimension(:,:), allocatable :: Minv_point !Metric tensor at each point
    !-------------------------
...
#include "parallel.h"
        ...
        integer :: i, j, unit
        _REAL_  :: force_constant, force_constant_d
        ...
        rewind 5
        read(5,nml=asm,iostat=ierr)
        if(ierr>0) call sander_bomb("asm_define (asm module)", &
                                    "asm namelist read error", &
    ...
    contains
        ...
        subroutine read_initial_guess
            ...
            integer :: i, u, ninit
            _REAL_, dimension(:,:), allocatable :: string_init
            _REAL_, dimension(msize) :: Mtmp, Mtmpinv
        ...
        subroutine interpolate_linear(A, B, M)
            ...
            _REAL_, dimension(:,:), intent(out) :: B
        
            integer :: ninit, nfinal
            _REAL_  :: Ltotal, Lnew
            _REAL_, dimension(size(A, dim=2)) :: L
            _REAL_, dimension(size(A, dim=2),size(A, dim=2)) :: A_cont !continuous string (no jumps because of periodic coordinates)
    ...
    !==================================================================
    subroutine asm_calc(x, force, energy)
    
        _REAL_, dimension(:), intent(in) :: x
        _REAL_, dimension(:), intent(inout) :: force
        _REAL_, intent(out) :: energy

        _REAL_ :: dpos_tmp, dK_tmp, pos_target, delta, sigma2, sigma2_target
        _REAL_ :: s, z
        _REAL_, dimension(nCV) :: dz_tmp
    ...
    contains
        ...
        _REAL_ function scale_dpos(x) !Ensures that the node positions never cross
        ...
        subroutine average_string(first, last)
            ...
            integer :: i, u, n
            _REAL_, dimension(nnodes) :: params_tmp
            _REAL_, dimension(nCV) :: reference
            _REAL_, dimension(nCV, nnodes) :: string_tmp
            character*200 :: si
...
#include "../include/memory.h"        
            _REAL_, dimension(natom*3) :: grad_s, grad_z
        ...
        subroutine calc_grote_hynes(grad_s)
            ...
            _REAL_, dimension(natom*3), intent(in) :: grad_s

            integer :: i
            _REAL_ :: sqm !sqrt(m)
            _REAL_ :: dqm2 !|dq|**2 in mass-weighted cartesians
            _REAL_ :: mu !reduced mass
            _REAL_ :: f_q !projection of external force on q
            _REAL_ :: fi_q !projection of fi on q
            _REAL_ :: s, z !Not actually used, but needed as dummy
            _REAL_, dimension(3) :: dq_dxi !dq / dx_i mass-weighted
            _REAL_, dimension(3) :: fi ! - dV / dx_i mass-weighted
    ...
    subroutine write_convergence
        ...
        _REAL_, dimension(nCV,nnodes) :: tmp_string
    ...
    contains
        ...
        !----------------------------------------
        _REAL_ function strings_distance(A, B, M_list)
    
            _REAL_, dimension(:,:), intent(in) :: A, B
            _REAL_, dimension(:,:), intent(in) :: M_list
    ...
    subroutine update_B
        ...
        _REAL_, dimension(nCV) :: Minvn
        _REAL_, dimension(msize) :: S
    ...
    subroutine REX
        ...
        _REAL_, dimension(buffer_size) :: buffer_tmp
        _REAL_, dimension(nCV,buffer_size) :: CV_buffer_tmp
    ...
    contains
        ...
        !---------------------------------------
        _REAL_ function prob(i) !Calculates the exchange probability between nodes i and i+1
        ...
        !---------------------------------------
        _REAL_ function node_energy(node_idx, CV_idx)
    ...
    subroutine reparameterize_linear
        ...
        integer :: i, j
        _REAL_, dimension(nnodes) :: L
        _REAL_, dimension(nCV) :: dz
    ...
    subroutine reparameterize_splines
        ...
        integer :: i
        _REAL_, dimension(nnodes) :: x !Initial parameterization (arbitrary, node indices are used here)
        _REAL_, dimension(nnodes) :: L !arc lengths up to each node
        _REAL_, dimension(nnodes) :: string_der !|dz/dx|_M - string derivative length with metric Minv
        _REAL_, dimension(nnodes-1,5) :: string_der_spline !|dz/dx|_M as function of x (to calculate the arc length)
        _REAL_, dimension(nnodes-1,5) :: xofL_spline !x as function of arc length
    ...
    !Create splines interpolation for PMFCV decomposition
        ...
        _REAL_, dimension(:), intent(in) :: L !arc lengths up to each node
...
!------------------------------------------------------------------------------!
    ...
    subroutine update_path
        ...
        integer :: i, j
        _REAL_, dimension(nCV) :: neg_extra, pos_extra !Vectors to extrapolate the path
    ...
    subroutine string_PMF_calculate
        ...
        _REAL_ :: mean, sigma2, bin_width, mean_error, sigma2_error
        _REAL_ :: K, s0, tmp
        integer, dimension(nbins) :: N_bin !Number of points in each bin
        _REAL_, dimension(nbins) :: force, t_bin
        _REAL_, dimension(nbins) :: pop_bin !Bin populations
        _REAL_, dimension(nbins) :: w_error, force_error, PMF_error
        _REAL_, dimension(nCV) :: CVtmp
        _REAL_, dimension(:), allocatable :: s, z, w
    ...
    contains
        ...
        subroutine string_PMFCV_write(filename)
                    ...
                    _REAL_, dimension(nCV) :: coef !Coefficients for the decomposition (Minv*n)
                    _REAL_, dimension(nCV) :: tmp
                    _REAL_, dimension(nCV,nbins) :: forceCV, PMFCV, forceCV_error, PMFCV_error
...
#include "../include/memory.h"
        ...
        _REAL_, dimension(nCV), intent(in) :: CV
        _REAL_, intent(out) :: s
        _REAL_, intent(out), optional :: z
        _REAL_, dimension(natom*3,nCV), intent(in), optional :: Jacobian
        _REAL_, dimension(natom*3), intent(out), optional :: grad_s, grad_z

        integer :: i
        _REAL_ :: d, sumw
        
        _REAL_, dimension(npointstotal) :: w
        _REAL_, dimension(nCV) :: dCV
        _REAL_, dimension(nCV,npointstotal) :: grad_tmp !temporary values for gradient calculation
