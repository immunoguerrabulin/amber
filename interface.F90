...
! end type potential_energy_rec
   ...
   type :: sander_input
      ...
      double precision :: fires_k  !JOSE MOD
      ...
      integer :: lj1264
      integer :: fires
      double precision :: mts_fires   !JOSE MOD: FIRES small timestep (ps)
      integer          :: mts_n       !JOSE MOD: FIRES microstep count per outer step
      ...
      character(len=256) :: fire_inner
      character(len=256) :: fire_inner_solvent
      character(len=256) :: fire_outer
...
subroutine gas_sander_input(inp, gb)
   ...
   inp%fires = 0
   inp%mts_fires = 0.0d0   !JOSE MOD: default FIRES small timestep disabled
   inp%mts_n     = 1       !JOSE MOD: default no subcycling
   inp%alpb = 0
   inp%vdwmeth = 0
   inp%ew_type = 0
   inp%inp = 0
   ...
   inp%fires_k = 0.d0  !JOSE MOD
   inp%restraintmask = ''
   inp%bellymask = ''
   inp%refc = ''
   inp%fire_inner = ''            !JOSE MOD
   inp%fire_inner_solvent = ''    !JOSE MOD
   inp%fire_outer = ''            !JOSE MOD
   inp%ala = 0.d0
   inp%arg = 0.d0
   inp%asn = 0.d0
   inp%asp = 0.d0
