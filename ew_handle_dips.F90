...
subroutine handle_induced(crd,numatoms,iac,ico,charge, &
   ...
   implicit none
   integer numatoms,iac(*),ico(*),ibgwat,nres,ipres(*),cn7(*),C4Pairwise !New2021
   _REAL_ crd(3,*),charge(*),cn1(*),cn2(*),cn6(*),cn8(*), &
          eelt,epol,frc(3,*),xr(3,*),pol(*),virvsene, & !New2021
   ...
   if ( indmeth < 3 .or. initdip == 1 )then
      ...
      do iter = 1,maxiter
         call ewald_force(crd,numatoms,iac,ico,charge, &
               cn1,cn2,cn6,cn7,cn8,C4Pairwise,eelt,epol,frc,x,ix,ipairs, &
                xr,virvsene,pol, pol2, .false., & ! New2021
#ifdef HAS_10_12
                cn3,cn4,cn5,asol,bsol)
#else
                cn3,cn4,cn5)
