...
subroutine rdparm1(nf)
   ...
   use parms, only : numbnd, C4Pairwise, numang, nptra, nphb, nttyp ! New2021
...
#  include "ew_cntrl.h"
   ...
   character(len=80) fmtin,ifmt,afmt,rfmt, mfmt, nfmt,type ! New2021
   ...
   read(nf,fmt) natom,ntypes,nbonh,mbona,ntheth,mtheta,nphih,mphia, &
         ...
         mbper,mgper,mdper,ifbox,nmxrs,ifcap,numextra,ncopy,C4Pairwise ! New2021
...
#  endif /* API */
   ...
   write(6,8118) natom,ntypes,nbonh,mbona,ntheth,mtheta,nphih,mphia, &
         ...
         numang,nptra,natyp,nphb,ifbox,nmxrs,ifcap,numextra,ncopy ! New2021
   8118 format(t2, &
   ...
   write(6,8119) natom,ntypes,nbonh,mbona,ntheth,mtheta,nphih,mphia, &
         nhparm,nparm,nnb,nres,nbona,ntheta,nphia,numbnd, &
         numang,nptra,natyp,nphb,ifbox,nmxrs,ifcap,numextra,ncopy,C4Pairwise ! New2021
   8119 format(t2, &
         'NATOM  = ',i7,' NTYPES = ',i7,' NBONH = ',i7,' MBONA  = ',i7, &
         /' NTHETH = ',i7,' MTHETA = ',i7,' NPHIH = ',i7,' MPHIA  = ',i7, &
         /' NHPARM = ',i7,' NPARM  = ',i7,' NNB   = ',i7,' NRES   = ',i7, &
         ...
         /' NUMANG = ',i7,' NPTRA  = ',i7,' NATYP = ',i7,' NPHB   = ',i7, &
         /' IFBOX  = ',i7,' NMXRS  = ',i7,' IFCAP = ',i7,' NEXTRA = ',i7, &
         /' NCOPY  = ',i7,' C4Pairwise = ',i7/) ! New2021
   end if
#endif /* API */
   
   nttyp = ntypes * (ntypes + 1) / 2 ! number of LJ type pairs TODO: C4?
   ! New2021 write(*,*) nttyp
   ! write(*,*) C4Pairwise 
...
#  include "ew_mpole.h"
   ...
   cn1 = cn1*vdwscale ! R. Luo for pGM vdw scaling
   ...
   cn2 = cn2*vdwscale ! R. Luo for pGM vdw scaling
...
!  New2021
   if (plj1264 /= 0) then
      ...
      if (iok /= 0) then
         ...
         !else
            ! Requested, but not present
            !CHECK_REQUIRED(iok, 'LENNARD_JONES_DCOEF')
         end if
      else
         read(nf, fmt) (cn7(i), i= 1,3*C4Pairwise)
         ! This was either requested, or is default when D-coefficients present
         plj1264 = 1
      end if
      if (plj1264 /= 0) then
         fmtin=nfmt
         ...
         if (iok /= 0) then
            if (plj1264 == -1) then 
               plj1264 = 0
            !else
               !CHECK_REQUIRED(iok, 'LENNARD_JONES_DVALUE')
            end if
         else
            read(nf, fmt) (cn8(i), i=1,C4Pairwise)
            plj1264 = 1
         end if     
      end if
   end if
