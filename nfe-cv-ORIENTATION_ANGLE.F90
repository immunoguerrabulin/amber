...
module nfe_cv_ORIENTATION_ANGLE
...
NFE_MASTER_ONLY_BEGIN
   if ( priv%quaternion(1,1)**2 < ONE ) then
      tmp = (180.0/PI)*(-2.0)* fcv / &
             max(sqrt(ONE - (priv%quaternion(1,1))**2),NFE_TO_REAL(0.00000001))
   else
      tmp = ZERO
   end if
 !     NFE_MASTER_ONLY_BEGIN
...
subroutine colvar_bootstrap(cv, cvno, amass)
   ...
   type(colvar_t), intent(inout) :: cv
   NFE_REAL, pointer :: coor(:) => null()         ! array containing all coordinates from refcrd file
...
#  include "nfe-mpi.h"
   ...
   ! check if reference coordinates are defined                     ! XX This doesn't work. It just sets zero for undefined ones   
   if (cv_nr.gt.0) then
   ...
   ! Read reference coordinates from a reference file  
   if (cv_nr.eq.0) then
      if (associated(coor)) &
          deallocate(coor)
          ...
          if (error.ne.0) &
           NFE_OUT_OF_MEMORY
           call read_refcrd(coor, trim(refcrd_file))            !refcrd_file is public variable 
...
end module nfe_cv_ORIENTATION_ANGLE
