...
#  endif /* MPI */
  ...
  use pol_gauss_interface, only : POL_GAUSS_readparm, POL_GAUSS_deallocate, &
                      POL_GAUSS_print_dipole
#ifdef MPI
  use pol_gauss_interface, only : POL_GAUSS_bcast
#endif
...
#  define rem 0
  ...
  ! ASM
#ifdef MPI
  use asm_mod, only : asm, asm_define
#endif
  ...
  use dprcmod, only : new_dprc_iface

  use dlfind_module, only : run_dlfind
...
#  endif
  ...
  masterwork: if (master) then
      if (abfqmmm_param%abfqmmm == 0) then
        ...
        call POL_GAUSS_readparm(8, natom, nstlim)
      end if
...
#  endif
      ...
      if ( ( qmmm_nml%ifqnt .and. &
         ...
         use_cew = .true.
         ! This used to be necessary, but it appear to work fine
         ! after rearranging the calling sequence
         ...
         !       write(6,*) 'WARNING: Automatically setting qm_pme=.false.'
    ...
    !
    ! DPRc non-MPI setup. The MPI setup is in the MPI-block, below.
    ! We need to wait for the global ih and ix arrays to be properly
    ! initialized on on sanderrank(s) > 1 before we can initialize DPRc.
    ! The DPRc initialization must be performed before the control
    ! flow diverges from mpi_orig = .true.
    !
#ifndef MPI
#ifdef DPRC
    call new_dprc_iface()
    ...
    ! polarizable Gaussian Multipole parameters
    call mpi_bcast(ipgm, 1, mpi_integer, 0, commsander, ier)
    if ( ipgm /= 0 ) then
      call POL_GAUSS_bcast(natom)
    end if
...
#  endif /* LES */
    ...
    if (ifsc .ne. 0) then
      ...
      if (imin > 0 .and. numtasks > 1 .and. imin .ne. 6 ) then
...
#ifndef LES
#ifdef CEW
      call mpi_bcast(use_cew, 1, mpi_logical, 0, commsander, ier)
#endif
#endif
    ...
    notmastermpi_orig: if (mpi_orig .and. .not. master) then
      ...
      if ( gbsa == 3 ) then
        write(6,*) "Called gbsa3_init"
        call gbsa3_init(natom, nres, residue_pointer, nbona, nbonh,&
             bonds_inc_hydrogen_1, bonds_inc_hydrogen_2, bonds_without_hydrogen_1, bonds_without_hydrogen_2, &
             atomicnumber, residue_label, atom_name)
      end if
      if (ifcr .ne. 0) then
    ...
    ! ASM
#ifdef MPI
    if (asm == 1) call asm_define(x(lcrd:lcrd+natom*3-1))
#endif
...
#  endif
      ...
      case (1)
        ...
        ntmincase: select case (ntmin)
          ...
          case (5)
            ! write(6,'(a)') 'DL-Find Optimization'
            ! write(6,'(a)') ''
            ! write(6,'(a)') 'J. Kastner, J. M. Carr, T. W. Keal, W. Thiel, A. Wander,'
            ! write(6,'(a)') 'and P. Sherwood, J. Phys. Chem. A, 2009, 113 (43),'
            ! write(6,'(a)') '11856-11865.'
            ! write(6,'(a)') ''
            ! write(6,'(a)') 'This version was taken from Py-ChemShell (v21.0.1) with'
            ! write(6,'(a)') 'library interfaces written by Kjell Jorner, distributed'
            ! write(6,'(a)') 'at the Digital Chemistry Laboratrory github page:'
            ! write(6,'(a)') 'https://github.com/digital-chemistry-laboratory/libdlfind'
            ! write(6,'(a)') ''
            
            call run_dlfind( x(lcrd), x(lforce), ene, qsetup )
            ! Do not write the restart file here
            ! The output coordinates do not necessarily correspond to
            ! stationary point
  ...
  if (master) then
    ...
    if ( ipgm > 0 ) then
      if ( dipole_print /= 0 ) call POL_GAUSS_print_dipole()
    end if
  end if
  call amflsh(6)
