...
subroutine firstbox_from_crd(natom,crd)
   !
   ! Recalculates the virtual box used for gas phase calculations
   ! (i.e., ntb=0, igb=0, ipb=0)
   ! to avoid the "The system has extended beyond the extent of the virtual box."
   ! error message. The error suggests a simple restart will fix the problem
   ! by recomputing the virtual box. The idea here is to simply recompute the
   ! box without terminating.
   !
   ! This function should never be called if periodic boundary conditions
   ! are used.
   ! 
   use nblist, only: cutoffnb,a,b,c,alpha,beta,gamma
   implicit none
! #  include "../include/md.h"
#  include "ew_cntrl.h"
#  include "extra.h"
...
!#endif
   ...
   extents(1:3,1) =  crd(1:3,1)
   extents(1:3,2) =  crd(1:3,1)
   do i=2,natom
      do j=1,3
         extents(j,1)=min(extents(j,1),crd(j,i))
         extents(j,2)=max(extents(j,2),crd(j,i))
      end do
   end do
   ...
   ! we have new lattice vector lengths
   ! recompute unit cell information

   call fill_ucell(a,b,c,alpha,beta,gamma)
   
   return
 end subroutine firstbox_from_crd
