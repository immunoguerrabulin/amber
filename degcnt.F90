...
#endif /* LES */
#ifdef MPI /* SOFT CORE */
   use softcore, only : sc_dof_shaked, sc_dof_belly, ifsc
#  ifndef LES
...
#ifdef LES
#else
            ! Check if this belly atom lies in the softcore region
            if (ifsc /= 0) then
                 if (nsc(i) == 1 ) sc_dof_belly  = sc_dof_belly  + 3
            end if
# endif
# endif
         end if
...
# ifdef MPI /* SOFT CORE */
            ! Check if this belly atom lies in the softcore region
            if (ifsc /= 0) then
                 if (nsc(ib) == 1 ) sc_dof_belly  = sc_dof_belly  + 3
            end if
# endif
#endif
         end if  ! (ibelly <= 0)
