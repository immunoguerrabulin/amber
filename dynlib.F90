...
#  endif
  ...
  use external_module, only : extprog
...
#  include "tgtmd.h"
  ...
  if ( dprc_nml%idprc > 0 ) then
     write(6,9151)ener%pot%ml
  end if

  if ((iextpot > 0) .and. (Upcase(extprog) .eq. 'TORCHANI')) then
     ! 9152 format(1x, 'EANI =', es30.15E4)
     write(6,9152) epot
  endif
...
#  endif
  ...
  9101 format (1x,'XTBESCF= ',f14.4)
  9102 format (1x,'DFTBPLUSESCF= ',f14.4)
...
#  include "tgtmd.h"
  ...
  if (qmmm_nml%ifqnt) then
    ...
    if (qmmm_nml%qmtheory%PM3) then
      ...
      write(ounit, 9101) escf
    else if (qmmm_nml%qmtheory%ISDFTBPLUS) then
      write(ounit, 9102) escf
    else
      write(7,'(" ERROR - UNKNOWN QM THEORY")')
    end if
  end if
