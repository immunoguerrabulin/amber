...
! modules used:  {{{
  ...
  use md_scheme, only: ischeme, thermostat_step, baro_stochastic, therm_vol
...
#  endif /* LES */
  use softcore, only: ifsc, sc_dvdl, sc_tot_dvdl, sc_tot_dvdl_partner, &
                      ...
                      ijarzynski, jarzynski_work, &
                      update_jarzynski_work, reset_jarzynski_work
...
#include "../pbsa/pb_md.h"
  ...
  _REAL_ dtx, dtxinv, dt5, dt25, factt, ekin0, ekinp0, dtcp, dttp
...
#  endif
  ...
  _REAL_ :: Nkt,kT,centvir,pressure, aa, arg2, poly, e2, e4, e6, e8
  _REAL_ :: box_center(3)

  ! variables used in middle scheme
  _REAL_ :: xold(3*natom)       ! for constrained MD
  _REAL_ :: x_centroid(3,natom) ! for PIMD
  _REAL_ :: dtcres, cell_ln(3), ekcmt_nkt ! for NPT Middle Scheme MD (in C-rescale)
  _REAL_ :: c1v, c2v! for NPT Middle Scheme PIMD (in MTTK)
...
!------------------------------------------------------------------------------
  ...
  if (barostat == 1 .and. baro_stochastic == 1) then
    dtcres = 2.0 * dtcp * 8.31441d2 * temp0  / 6.023d0
    ekcmt_nkt = nspm * 8.31441d-3 * temp0 * 0.5 / 4.184
  end if
    ! }}}
...
!------------------------------------------------------------------------------
    ...
    ! FIRES forces/energy are handled centrally inside force.F90; subcycling handled here  !JOSE MOD
    ...
    if (ntp > 0) then
      ...
      if (iamoeba == 0) then
        ...
        do m = 1,3
          ...
          if(baro_stochastic .eq. 1) then
            ener%pres(m) = (pconv+pconv) * (ekcmt_nkt-ener%vir(m)) / volume
          else
...
!------------------------------------------------------------------------------
  ...
  if (ntp > 0 .and. ipimd > 0) then
    ...
    if(ischeme == 1) then
      c1v = exp(-dt*therm_vol)
      c2v = sqrt(1.0d0 - c1v*c1v)*sqrt(2.d0*boltz2*temp0/mass_lnv)
    end if
  end if
...
!---------- BEGIN FIRES MTS ---------------------------------------------------
  ...
  !JOSE Notes (FIRES MTS): If enabled, run an inner subcycling integrator with
  ! small-step delta = mts_fires (ps), refreshing FIRES each microstep while
  ! keeping the slow force cached from the call above. Restrict to simple
  ...
  ! Allocate temporaries for local degrees of freedom
      ...
      if (fires_magnitude < 1.0d-12) then
         ...
         do j = istart, iend
            ...
            !write(6,'(i6,6es12.3)')j,v(i3+1:i3+3),f_slow(i3+1:i3+3)
            v(i3+1) = v(i3+1) + f_slow(i3+1) * wfac
            v(i3+2) = v(i3+2) + f_slow(i3+2) * wfac
            v(i3+3) = v(i3+3) + f_slow(i3+3) * wfac
            i3 = i3 + 3
         end do
...
#  endif /* USE_MPI_IN_PLACE */
  ...
  if (ntp > 0) then
    ...
    if (iamoeba == 0 .and. ipimd == 0) then
      ...
      if (barostat == 2 .and. ipgm == 1) ener%pres = 0.0d0  ! MC barostat does not have pressure. YD
      ener%pres(4) = ener%pres(4) / 3.d0
...
!------------------------------------------------------------------------------
  ...
  ! the 1st step for updating p
     ...
     if(ntp > 0 .and. ipimd == NMPIMD) then
       v_lnv = v_lnv + dtx*(kT * 3.0d0+ f_lnv_p)/mass_lnv
       rmu(1:3) = exp(v_lnv*dt5)
...
!------------------------------------------------------------------------------
  ...
  if (ischeme == 1) then
    ...
    ! Second update Thermostat by dt, update barostat for v_lnv by dt
    ...
    ! add barostat
    ! Third update x by dt/2
    if(ntp > 0 .and. ipimd == NMPIMD) then
      call gauss(0.0d0, c2v, fln)
      if(mybeadid == 1 .and. master) then
        v_lnv = c1v * v_lnv + fln
      end if
#ifdef MPI
...
!------------------------------------------------------------------------------
  ...
  ! }}}
  if (iamoeba == 0 .and. barostat == 1 .and. ipimd == 0) then
    !  next, for non-amoeba: {{{
    if (ntp == 1) then

      if(baro_stochastic .eq. 1) then
        call gauss(0.d0, 1.0d0, cell_ln(1))
        if(master) then
          rmu(1) = exp(dtcp * (ener%pres(4) - pres0) + sqrt(dtcres/volume)*cell_ln(1))**third
        end if
#ifdef MPI
          call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
              commsander, ierr)
#endif
      else
      ...
      if (csurften > 0) then
        ...
        if(baro_stochastic .eq. 1) then
          call gauss(0.d0, 1.0d0, cell_ln(1))
          call gauss(0.d0, 1.0d0, cell_ln(2))
          call gauss(0.d0, 1.0d0, cell_ln(3))
          if (master) then
            rmu(1) = exp(dtcp * (ener%pres(1) - pres0x) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
            rmu(2) = exp(dtcp * (ener%pres(2) - pres0y) + sqrt(dtcres/volume/3.d0)*cell_ln(2))**third
            rmu(3) = exp(dtcp * (ener%pres(3) - pres0z) + sqrt(dtcres/volume/3.d0)*cell_ln(3))**third
          end if
#ifdef MPI
          call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
              commsander, ierr)
        ...
        if(baro_stochastic .eq. 1) then
          ...
          call gauss(0.d0, 1.0d0, cell_ln(3))
          if(master) then
            rmu(1) = exp(dtcp * (ener%pres(1) - pres0) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
            rmu(2) = exp(dtcp * (ener%pres(2) - pres0) + sqrt(dtcres/volume/3.d0)*cell_ln(2))**third
            rmu(3) = exp(dtcp * (ener%pres(3) - pres0) + sqrt(dtcres/volume/3.d0)*cell_ln(3))**third
          end if
#ifdef MPI
          call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
                 commsander, ierr)
      ...
      if (csurften > 0) then
        if (csurften == 1) then
          ...
          if(baro_stochastic .eq. 1) then
            ...
            if(master) then
              rmu(1) = exp(dtcp * (ener%pres(1) - pres0x) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
              rmu(2) = exp(dtcp * (press_tan_ave - pres0y) + sqrt(dtcres/volume/3.d0)*cell_ln(2))**third
              rmu(3) = exp(dtcp * (press_tan_ave - pres0z) + sqrt(dtcres/volume/3.d0)*cell_ln(3))**third
            end if
#ifdef MPI
            call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
                   commsander, ierr)
          ...
          if(baro_stochastic .eq. 1) then
            ...
            call gauss(0.d0, 1.0d0, cell_ln(3))
            if(master) then
              rmu(1) = exp(dtcp * (press_tan_ave - pres0x) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
              rmu(2) = exp(dtcp * (ener%pres(2) - pres0y) + sqrt(dtcres/volume/3.d0)*cell_ln(2))**third
              rmu(3) = exp(dtcp * (press_tan_ave - pres0z) + sqrt(dtcres/volume/3.d0)*cell_ln(3))**third
            end if
#ifdef MPI
        ...
        ! surface tension here
          if (csurften == -1) then        ! For pressure control in x
            ...
            if(baro_stochastic .eq. 1) then
              call gauss(0.d0, 1.0d0, cell_ln(1))
              if(master) then
                  rmu(2) = exp(dtcp * (ener%pres(2) - pres0y) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
              end if
#ifdef MPI
              call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
                  commsander, ierr)
          ...
          !else if (csurften == 3) then   ! For pressure control in z
            ...
            if(baro_stochastic .eq. 1) then
              call gauss(0.d0, 1.0d0, cell_ln(1))
              if(master) then
                  rmu(3) = exp(dtcp * (ener%pres(3) - pres0z) + sqrt(dtcres/volume/3.d0)*cell_ln(1))**third
              end if
#ifdef MPI
              call mpi_bcast(rmu, 3, MPI_DOUBLE_PRECISION, 0, &
                  commsander, ierr)
    ...
    ! }}}
  else if (barostat == 1 .and. ipimd == 0) then
    ! finally, for amoeba: {{{
    if (ntp > 0) then
    ...
    if (ntave > 0) then
      if (mod(total_nstep,ntave) == 0 .and. onstep) then
        ...
        if ( ijarzynski > 0 ) then
           call update_jarzynski_work(ener)
        end if
#endif
...
!----------------------------------------------------------------------------
  ! Jarzynski analysis post-dynamics report
  if ( ijarzynski > 0 .and. master ) then
     write(6,'(/,a,f16.6,/)')"Final work value: ",jarzynski_work
  end if
