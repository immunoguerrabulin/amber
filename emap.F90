...
#  include "../include/md.h"
      ...
      INTEGER I,J,nm,iat,i3,j3,idemp,grids(6)
      INTEGER alloc_err,UNITOUT
      _REAL_ RESO,gridsize
...
1220  FORMAT(" ----------------------- END OF EMAP IMAGE ",I4, &
      ...
      SUBROUTINE BOUNDARYMAP(IDEMP,DGRID,GRIDS,NATC,AMASS,CRD,OUTU)
        ...
        !
              ...
              INTEGER IAT
        !
              INTEGER I,J,K,M1,M2,M3,NDATA,IXYZ
              _REAL_ XMIN,YMIN,ZMIN,XMAX,YMAX,ZMAX
              REAL*4 dens1,dens2
              INTEGER alloc_err
        ...
        !
              ...
              write(6,'(A,2F10.2)')"WARNING: Boundary map is too small in x direction!",(xmax-xmin),grids(1)*dgrid
              if((ymax-ymin)>grids(2)*dgrid) &
              write(6,'(A,2F10.2)')"WARNING: Boundary map is too small in y direction!",(ymax-ymin),grids(2)*dgrid
              if((zmax-zmin)>grids(3)*dgrid) &
              write(6,'(A,2F10.2)')"WARNING: Boundary map is too small in z direction!",(zmax-zmin),grids(3)*dgrid
              HX=(grids(1)/2+1)
              HY=(grids(2)/2+1)
              HZ=(grids(3)/2+1)
              HX2=HX*HX
