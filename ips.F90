...
!
      use constants
      use amoeba_mdin, only : iamoeba,ee_dsum_cut_am=>ee_dsum_cut
      use pol_gauss_mdin, only : ipgm,ee_dsum_cut_pgm=>ee_dsum_cut
...
!
        ...
        if(ipgm==1)then
          TEIPS=.FALSE.
          RIPS=ee_dsum_cut_pgm
          RIPS2=RIPS*RIPS
          AMIPS0(0) = -35.0d0/16.0d0
          AMIPS0(1) =  35.0d0/16.0d0
          AMIPS0(2) = -21.0d0/16.0d0
          AMIPS0(3) =  5.0d0/16.0d0
          AMIPS0(4) =  0.0d0
          AMIPS0(5) =  0.0d0
        endif
...
!
   ...
   SUBROUTINE pGM_FIPSMDQ(R,ERF_BR,BETA,FIPS0,FIPS1,FIPS2,FIPS3)
   _REAL_,intent(in)  :: R,ERF_BR,BETA
   _REAL_,intent(out)  ::  FIPS0,FIPS1,FIPS2,FIPS3
   _REAL_ U,U2,U4,R2,R4,BR,BR2,BR3,BR5,EXP_BR2
  _REAL_ :: PIROOT = SQRT(3.14159265358979)
   R2=R*R
   R4=R2*R2
   U=R*RIPSR
   U2=U*U
   ...
   BR3  = BR2  * BR
   BR5  = BR3  * BR2 
   EXP_BR2  = exp(-BR2) /PIROOT
   FIPS0=(ERF_BR+U*(AMIPS0(0)+U2*(AMIPS0(1)+U2*(AMIPS0(2)      &
         +U2*(AMIPS0(3)+U2*(AMIPS0(4)+U2*(AMIPS0(5))))))))/R
   FIPS1=(-(ERF_BR-2.0d0*BR * EXP_BR2)+U*U2*(AMIPS1(0)+U2*(AMIPS1(1)+U2*(AMIPS1(2)  &
         +U2*(AMIPS1(3)+U2*(AMIPS1(4)))))))/R/R2
   FIPS2=((3.0d0*ERF_BR-(6.0d0*BR+4.0d0*BR3) * EXP_BR2)+U*U4*(AMIPS2(0)+U2*(AMIPS2(1)+U2*(AMIPS2(2) &
         +U2*(AMIPS2(3))))))/R/R4
   FIPS3=(-(15.0d0*ERF_BR-(30.0d0*BR+20.0d0*BR3+8.0d0*BR5) * EXP_BR2)+U*U2*U4*(AMIPS3(0)+U2*(AMIPS3(1)+U2*(AMIPS3(2) &
         ))))/R/R2/R4
   !FIPS4=(105.0D0*WEIGHT+U*U4*U4*(AMIPS4(0)+U2*(AMIPS4(1))))/R/R4/R4
   !FIPS5=(-945.0D0*WEIGHT+U*U2*U4*U4*(AMIPS5(0)))/R/R2/R4/R4
   RETURN
   END SUBROUTINE pGM_FIPSMDQ

   SUBROUTINE pGM_FIPSMDQ0(FIPS0,FIPS1,FIPS2)
   _REAL_ FIPS0,FIPS1,FIPS2
   FIPS0=AMIPS0(0)*RIPSR
   FIPS1=AMIPS1(0)*RIPSR*RIPS2R
   FIPS2=AMIPS2(0)*RIPS6R*RIPS
   RETURN
   END SUBROUTINE pGM_FIPSMDQ0
